...
namespace asio {
  ...
  class mutable_buffer {
    ...
    mutable_buffer() ASIO_NOEXCEPT : data_(0), size_(0) {
    }

    /// Construct a buffer to represent a given memory range.
    mutable_buffer(void *data, std::size_t size) ASIO_NOEXCEPT : data_(data),
                                                                 size_(size) {
    }
    ...
    mutable_buffer(void *data,
        ...
        : data_(data), size_(size), debug_check_(debug_check) {
    }
  ...
  /// (Deprecated: Use mutable_buffer.) Adapts a single modifiable buffer so
  /// that it meets the requirements of the MutableBufferSequence concept.
   ...
   * cannot be modified. It does not own the underlying data, and so is cheap to
   * copy or assign.
   *
   * @par Accessing Buffer Contents
   *
  ...
  class const_buffer {
    public:
    /// Construct an empty buffer.
    const_buffer() ASIO_NOEXCEPT : data_(0), size_(0) {
    }
  ...
  /// Trait to determine whether a type satisfies the DynamicBuffer
  /// requirements.
  template <typename T>
  ...
  namespace detail {
    ...
    struct buffer_sequence_cardinality
        : conditional<is_same<BufferSequence, mutable_buffer>::value
#if !defined(ASIO_NO_DEPRECATED)
                          ||
                      ...
                      one_buffer,
                      multiple_buffers>::type {
    };

    template <typename Iterator>
    inline std::size_t
    buffer_size(one_buffer, Iterator begin, Iterator) ASIO_NOEXCEPT {
      return const_buffer(*begin).size();
    }

    template <typename Iterator>
    inline std::size_t
    buffer_size(multiple_buffers, Iterator begin, Iterator end) ASIO_NOEXCEPT {
  ...
  inline mutable_buffer operator+(const mutable_buffer &b,
    ...
    return mutable_buffer(new_data,
                          new_size
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
                          ,
                          b.get_debug_check()
  ...
  namespace detail {
    ...
    class buffer_debug_check {
      public:
      buffer_debug_check(Iterator iter) : iter_(iter) {
      }

      ~buffer_debug_check() {
#if defined(ASIO_MSVC) && (ASIO_MSVC == 1400)
        // MSVC 8's string iterator checking may crash in a
        // std::string::iterator object's destructor when the iterator points to
        // an already-destroyed std::string object, unless the iterator is
        // cleared first.
        iter_ = Iterator();
   ...
   * size_t}</tt> specifies a mutable (modifiable) region of memory. Similarly,
   * a tuple of the form <tt>{const void*, size_t}</tt> specifies a const
   * (non-modifiable) region of memory. These two forms correspond to the
   * classes mutable_buffer and const_buffer, respectively. To mirror C++'s
   * conversion rules, a mutable_buffer is implicitly convertible to a
   * const_buffer, and the opposite conversion is not permitted.
   *
   ...
   * is the responsibility of the application to ensure the memory region
   * remains valid until it is no longer required for an I/O operation. When the
   * memory is no longer available, the buffer is said to have been invalidated.
   *
   ...
   * empty.
   *
   * Both an offset and size may be specified to create a buffer that
   * corresponds to a specific range of bytes within an existing buffer:
   *
   ...
   * MutableBufferSequence (for read) or ConstBufferSequence (for write)
   * concepts:
   *
   * @code
   * char d1[128];
...
#define ASIO_CONST_BUFFER const_buffers_1
   ...
   */
  inline ASIO_MUTABLE_BUFFER buffer(const mutable_buffer &b) ASIO_NOEXCEPT {
   ...
   */
  inline ASIO_MUTABLE_BUFFER
  buffer(const mutable_buffer &b, std::size_t max_size_in_bytes) ASIO_NOEXCEPT {
    return ASIO_MUTABLE_BUFFER(mutable_buffer(
        b.data(),
   ...
   */
  inline ASIO_CONST_BUFFER buffer(const const_buffer &b) ASIO_NOEXCEPT {
  ...
  inline ASIO_CONST_BUFFER buffer(const const_buffer &b,
    ...
    return ASIO_CONST_BUFFER(b.data(),
                             b.size() < max_size_in_bytes ? b.size()
                                                          : max_size_in_bytes
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
                             ,
                             b.get_debug_check()
  ...
  template <typename PodType, std::size_t N>
  inline ASIO_MUTABLE_BUFFER
  buffer(PodType (&data)[N], std::size_t max_size_in_bytes) ASIO_NOEXCEPT {
   ...
   */
  template <typename PodType, std::size_t N>
  inline ASIO_CONST_BUFFER buffer(const PodType (&data)[N]) ASIO_NOEXCEPT {
  ...
  namespace detail {
    ...
    struct buffer_types : public buffer_types_base<is_const<PodType>::value> {};
  ...
  buffer(boost::array<PodType, N> &data) ASIO_NOEXCEPT {
    typedef
        typename asio::detail::buffer_types<PodType>::buffer_type buffer_type;
   ...
   */
  template <typename PodType, std::size_t N>
  inline ASIO_CONST_BUFFER
  buffer(boost::array<const PodType, N> &data) ASIO_NOEXCEPT {
  ...
  template <typename PodType, std::size_t N>
  inline ASIO_CONST_BUFFER
  buffer(const boost::array<PodType, N> &data) ASIO_NOEXCEPT {
    return ASIO_CONST_BUFFER(data.data(), data.size() * sizeof(PodType));
  }
  ...
  template <typename PodType, std::size_t N>
  inline ASIO_MUTABLE_BUFFER
  buffer(std::array<PodType, N> &data) ASIO_NOEXCEPT {
  ...
  template <typename PodType, std::size_t N>
  inline ASIO_CONST_BUFFER
  buffer(std::array<const PodType, N> &data) ASIO_NOEXCEPT {
    return ASIO_CONST_BUFFER(data.data(), data.size() * sizeof(PodType));
  }
  ...
  template <typename PodType, std::size_t N>
  inline ASIO_CONST_BUFFER
  buffer(const std::array<PodType, N> &data) ASIO_NOEXCEPT {
    return ASIO_CONST_BUFFER(data.data(), data.size() * sizeof(PodType));
  }
  ...
  inline ASIO_MUTABLE_BUFFER
  buffer(std::vector<PodType, Allocator> &data) ASIO_NOEXCEPT {
  ...
  template <typename PodType, typename Allocator>
  inline ASIO_CONST_BUFFER
  buffer(const std::vector<PodType, Allocator> &data) ASIO_NOEXCEPT {
   ...
   */
  template <typename PodType, typename Allocator>
  inline ASIO_CONST_BUFFER buffer(const std::vector<PodType, Allocator> &data,
                                  std::size_t max_size_in_bytes) ASIO_NOEXCEPT {
    return ASIO_CONST_BUFFER(
        data.size() ? &data[0] : 0,
        data.size() * sizeof(PodType) < max_size_in_bytes
  ...
  inline ASIO_MUTABLE_BUFFER
  buffer(std::basic_string<Elem, Traits, Allocator> &data) ASIO_NOEXCEPT {
    return ASIO_MUTABLE_BUFFER(
        data.size() ? &data[0] : 0,
  ...
  template <typename Elem, typename Traits, typename Allocator>
  inline ASIO_MUTABLE_BUFFER
  buffer(std::basic_string<Elem, Traits, Allocator> &data,
         std::size_t max_size_in_bytes) ASIO_NOEXCEPT {
    return ASIO_MUTABLE_BUFFER(
        data.size() ? &data[0] : 0,
  ...
  template <typename Elem, typename Traits, typename Allocator>
  inline ASIO_CONST_BUFFER
  buffer(const std::basic_string<Elem, Traits, Allocator> &data) ASIO_NOEXCEPT {
    return ASIO_CONST_BUFFER(
        data.data(),
        data.size() * sizeof(Elem)
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
            ,
        detail::buffer_debug_check<
            typename std::basic_string<Elem, Traits, Allocator>::
  ...
  template <typename Elem, typename Traits, typename Allocator>
  inline ASIO_CONST_BUFFER
  buffer(const std::basic_string<Elem, Traits, Allocator> &data,
         std::size_t max_size_in_bytes) ASIO_NOEXCEPT {
    return ASIO_CONST_BUFFER(
        data.data(),
        data.size() * sizeof(Elem) < max_size_in_bytes
  ...
  inline ASIO_CONST_BUFFER
  buffer(basic_string_view<Elem, Traits> data) ASIO_NOEXCEPT {
    return ASIO_CONST_BUFFER(
        data.size() ? &data[0] : 0,
        data.size() * sizeof(Elem)
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
            ,
        detail::buffer_debug_check<
            typename basic_string_view<Elem, Traits>::iterator>(data.begin())
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
    );
  }
   ...
   */
  template <typename Elem, typename Traits>
  inline ASIO_CONST_BUFFER buffer(basic_string_view<Elem, Traits> data,
                                  std::size_t max_size_in_bytes) ASIO_NOEXCEPT {
    return ASIO_CONST_BUFFER(
        data.size() ? &data[0] : 0,
        data.size() * sizeof(Elem) < max_size_in_bytes
  ...
  class dynamic_string_buffer {
    ...
    /**
     * @returns The allowed maximum of the sum of the sizes of the input
    ...
    /**
     * @returns The current total capacity of the buffer, i.e. for both the
    ...
    /// Get a list of buffers that represents the output sequence, with the
    /// given size.
    /**
   ...
   */
  template <typename Elem, typename Traits, typename Allocator>
  inline dynamic_string_buffer<Elem, Traits, Allocator>
  dynamic_buffer(std::basic_string<Elem, Traits, Allocator> &data,
   ...
   */
  template <typename Elem, typename Allocator>
  inline dynamic_vector_buffer<Elem, Allocator>
  dynamic_buffer(std::vector<Elem, Allocator> &data) ASIO_NOEXCEPT {
  ...
  template <typename Elem, typename Allocator>
  inline dynamic_vector_buffer<Elem, Allocator>
  dynamic_buffer(std::vector<Elem, Allocator> &data,
                 std::size_t max_size) ASIO_NOEXCEPT {
  ...
  namespace detail {
    ...
    inline std::size_t buffer_copy(one_buffer,
                                   ...
                                   SourceIterator) ASIO_NOEXCEPT {
      return (buffer_copy_1)(*target_begin, *source_begin);
    }

    template <typename TargetIterator, typename SourceIterator>
    inline std::size_t
    buffer_copy(one_buffer,
                one_buffer,
                TargetIterator target_begin,
                TargetIterator,
                SourceIterator source_begin,
    ...
    buffer_copy(one_buffer,
      ...
      for (mutable_buffer target_buffer(
        ...
        std::size_t bytes_copied =
            (buffer_copy_1)(target_buffer, source_buffer);
    ...
    std::size_t buffer_copy(multiple_buffers,
                            ...
                            TargetIterator target_end,
                            SourceIterator source_begin,
                            SourceIterator source_end) ASIO_NOEXCEPT {
      std::size_t total_bytes_copied = 0;
  ...
  /**
   * @param target A modifiable buffer sequence representing the memory regions
