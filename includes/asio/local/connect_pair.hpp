...
namespace asio {
  namespace local {
    ...
    void connect_pair(basic_socket<Protocol ASIO_SVC_TARG> &socket1,
                      basic_socket<Protocol ASIO_SVC_TARG1> &socket2);

    /// Create a pair of connected sockets.
    template <typename Protocol ASIO_SVC_TPARAM ASIO_SVC_TPARAM1>
    ASIO_SYNC_OP_VOID
    connect_pair(basic_socket<Protocol ASIO_SVC_TARG> &socket1,
                 basic_socket<Protocol ASIO_SVC_TARG1> &socket2,
                 asio::error_code &ec);

    template <typename Protocol ASIO_SVC_TPARAM ASIO_SVC_TPARAM1>
    inline void connect_pair(basic_socket<Protocol ASIO_SVC_TARG> &socket1,
                             basic_socket<Protocol ASIO_SVC_TARG1> &socket2) {
      asio::error_code ec;
