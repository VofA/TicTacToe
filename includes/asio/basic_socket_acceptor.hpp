...
namespace asio {
   ...
   * @endcode
   */
  template <typename Protocol ASIO_SVC_TPARAM_DEF1(
  ...
  class basic_socket_acceptor : ASIO_SVC_ACCESS basic_io_object<ASIO_SVC_T>,
    ...
    basic_socket_acceptor(asio::io_context &io_context,
      ...
      asio::detail::throw_error(ec, "open");
      if (reuse_addr) {
        this->get_service().set_option(
            this->get_implementation(), socket_base::reuse_address(true), ec);
        asio::detail::throw_error(ec, "set_option");
      }
      this->get_service().bind(this->get_implementation(), endpoint, ec);
      asio::detail::throw_error(ec, "bind");
      this->get_service().listen(
          this->get_implementation(), socket_base::max_listen_connections, ec);
    ...
    basic_socket_acceptor(asio::io_context &io_context,
      ...
      this->get_service().assign(
          this->get_implementation(), protocol, native_acceptor, ec);
     ...
     * if constructed using the @c basic_socket_acceptor(io_context&)
     * constructor.
     */
    ...
    operator=(basic_socket_acceptor<Protocol1 ASIO_SVC_TARG1> &&other) {
     ...
     * acceptor. This is intended to allow access to native acceptor
     ...
     * native acceptor. It has no effect on the behaviour of the acceptor
     * object's synchronous operations.
     *
     ...
     *
     * @returns An object that represents the local endpoint of the acceptor.
     * Returns a default-constructed endpoint object if an error occurred and
    ...
    void accept(
        ...
        typename enable_if<is_convertible<Protocol, Protocol1>::value>::type * =
            0)
#else  // defined(ASIO_ENABLE_OLD_SERVICES)
    ...
    ASIO_INITFN_RESULT_TYPE(AcceptHandler, void(asio::error_code))
    async_accept(
        basic_socket<Protocol1, SocketService> &peer,
     ...
     * @param peer The socket into which the new connection will be accepted.
     *
     * @param peer_endpoint An endpoint object which will receive the endpoint
     * of the remote peer.
     *
     * @throws asio::system_error Thrown on failure.
     *
...
#if defined(ASIO_ENABLE_OLD_SERVICES)
    template <typename SocketService>
    ASIO_SYNC_OP_VOID accept(basic_socket<protocol_type, SocketService> &peer,
                             endpoint_type &peer_endpoint,
                             asio::error_code &ec)
#else  // defined(ASIO_ENABLE_OLD_SERVICES)
    ...
    /// Accept a new connection.
    /**
     * This function is used to accept a new connection from a peer. The
     * function call will block until a new connection has been accepted
     * successfully or an error occurs.
     *
    ...
    typename Protocol::socket accept() {
      ...
      typename Protocol::socket peer(
          this->get_service().accept(this->get_implementation(), 0, 0, ec));
      asio::detail::throw_error(ec, "accept");
      return peer;
    }
     ...
     * @code void handler(
     *   const asio::error_code& error, // Result of operation.
     *   typename Protocol::socket peer // On success, the newly accepted
     * socket.
     * ); @endcode
     * Regardless of whether the asynchronous operation completes immediately or
     * not, the handler will not be invoked from within this function.
    ...
    async_accept(ASIO_MOVE_ARG(MoveAcceptHandler) handler) {
      ...
      ASIO_MOVE_ACCEPT_HANDLER_CHECK(
          MoveAcceptHandler, handler, typename Protocol::socket)
      type_check;
    ...
    typename Protocol::socket accept(asio::io_context &io_context,
                                     endpoint_type &peer_endpoint) {
      asio::error_code ec;
      typename Protocol::socket peer(this->get_service().accept(
          this->get_implementation(), &io_context, &peer_endpoint, ec));
      asio::detail::throw_error(ec, "accept");
      return peer;
    }
