...
namespace asio {
  ...
  struct is_match_condition {
    ...
    enum {
      value =
          asio::is_function<typename asio::remove_pointer<T>::type>::value ||
   ...
   * sequence until the dynamic buffer sequence's get area contains the
   * specified delimiter. The call will block until one of the following
   * conditions is true:
   *
   ...
   * stream's read_some function. If the dynamic buffer sequence's get area
   * already contains the delimiter, the function returns immediately.
   *
   * @param s The stream from which the data is to be read. The type must
   * support the SyncReadStream concept.
   *
   * @param buffers The dynamic buffer sequence into which the data will be
   * read.
   *
   * @param delim The delimiter character.
   *
   * @returns The number of bytes in the dynamic buffer sequence's get area up
   ...
   * After the @c read_until operation completes successfully, the string @c
   * data contains the delimiter:
   * @code { 'a', 'b', ..., 'c', '\n', 'd', 'e', ... } @endcode
   * The call to @c substr then extracts the data up to and including the
   * delimiter, so that the string @c line contains:
   * @code { 'a', 'b', ..., 'c', '\n' } @endcode
   * After the call to @c erase, the remaining data is left in the buffer @c b
   * as follows:
   * @code { 'd', 'e', ... } @endcode
   ...
   *
   * @returns The number of bytes in the dynamic buffer sequence's get area up
   * to and including the delimiter. Returns 0 if an error occurred.
   *
   * @note After a successful read_until operation, the dynamic buffer sequence
   * may contain additional data beyond the delimiter. An application will
   * typically leave that data in the dynamic buffer sequence for a subsequent
   ...
   * This operation is implemented in terms of zero or more calls to the
   * stream's read_some function. If the dynamic buffer sequence's get area
   * already contains data that matches the regular expression, the function
   * returns immediately.
   *
   * @param s The stream from which the data is to be read. The type must
   * support the SyncReadStream concept.
   ...
   *
   * @returns The number of bytes in the dynamic buffer sequence's get area up
   * to and including the substring that matches the regular expression.
   *
   * @throws asio::system_error Thrown on failure.
   *
   * @note After a successful read_until operation, the dynamic buffer sequence
   ...
   *
   * @returns The number of bytes in the dynamic buffer sequence's get area up
   * to and including the substring that matches the regular expression. Returns
   ...
   * to the data contained in the dynamic buffer sequence, indicates a
   successful
   * match. The call will block until one of the following conditions is true:
   *
   * @li The match condition function object returns a std::pair where the
   second
   ...
   * This operation is implemented in terms of zero or more calls to the
   stream's
   * read_some function. If the match condition function object already
   indicates
   ...
   * @param buffers A dynamic buffer sequence into which the data will be read.
   *
   * @param match_condition The function object to be called to determine
   whether
   ...
   * @c begin parameter for any subsequent invocation of the match condition.
   The
   * @c second member of the return value is true if a match has been found,
   false
   * otherwise.
   *
   ...
   * An application will typically leave that data in the dynamic buffer
   sequence
   ...
   *
   * @par Examples
   * To read data into a dynamic buffer sequence until whitespace is
   encountered:
   ...
   * sequence until a user-defined match condition function object, when applied
   * to the data contained in the dynamic buffer sequence, indicates a
   * successful match. The call will block until one of the following conditions
   * is true:
   *
   * @li The match condition function object returns a std::pair where the
   ...
   *
   * This operation is implemented in terms of zero or more calls to the
   * stream's read_some function. If the match condition function object already
   * indicates a match, the function returns immediately.
   *
   * @param s The stream from which the data is to be read. The type must
   * support the SyncReadStream concept.
   *
   ...
   *
   * @param match_condition The function object to be called to determine
   * whether a match exists. The signature of the function object must be:
   * @code pair<iterator, bool> match_condition(iterator begin, iterator end);
   * @endcode
   * where @c iterator represents the type:
   ...
   * may contain additional data beyond that which matched the function object.
   * An application will typically leave that data in the dynamic buffer
   * sequence for a subsequent read_until operation to examine.
   *
   * @note The default implementation of the @c is_match_condition type trait
   * evaluates to true for function pointers and function objects with a
   * @c result_type typedef. It must be specialised for other user-defined
   ...
   *
   * This operation is implemented in terms of zero or more calls to the
   * stream's read_some function. If the streambuf's get area already contains
   ...
   * @param delim The delimiter character.
   *
   * @returns The number of bytes in the streambuf's get area up to and
   ...
   * that data in the streambuf for a subsequent read_until operation to
   * examine.
   *
   * @par Example
   ...
   * application will typically leave that data in the streambuf for a
   * subsequent read_until operation to examine.
   *
   * @par Example
   ...
   *
   * @returns The number of bytes in the streambuf's get area up to and
   * including the substring that matches the regular expression. Returns 0 if
   * an error occurred.
   *
   * @note After a successful read_until operation, the streambuf may contain
   * additional data beyond that which matched the regular expression. An
   ...
   * false otherwise.
   *
   * @returns The number of bytes in the streambuf's get area that have been
   * fully consumed by the match function.
   *
   * @throws asio::system_error Thrown on failure.
   *
   * @note After a successful read_until operation, the streambuf may contain
   * additional data beyond that which matched the function object. An
   * application will typically leave that data in the streambuf for a
   * subsequent read_until operation to examine.
   *
   * @note The default implementation of the @c is_match_condition type trait
   ...
   *
   * @returns The number of bytes in the streambuf's get area that have been
   * fully consumed by the match function. Returns 0 if an error occurred.
   *
   * @note After a successful read_until operation, the streambuf may contain
   * additional data beyond that which matched the function object. An
   * application will typically leave that data in the streambuf for a
  ...
  /// Start an asynchronous operation to read data into a dynamic buffer
  /// sequence until it contains a specified delimiter.
  /**
   * This function is used to asynchronously read data into the specified
   * dynamic buffer sequence until the dynamic buffer sequence's get area
   * contains the specified delimiter. The function call always returns
   ...
   *
   * This operation is implemented in terms of zero or more calls to the
   * stream's async_read_some function, and is known as a <em>composed
   * operation</em>. If the dynamic buffer sequence's get area already contains
   * the delimiter, this asynchronous operation completes immediately. The
   ...
   *
   * @param s The stream from which the data is to be read. The type must
   * support the AsyncReadStream concept.
   *
   * @param buffers The dynamic buffer sequence into which the data will be
   * read. Although the buffers object may be copied as necessary, ownership of
   ...
   * delimiter, so that the string @c line contains:
   * @code { 'a', 'b', ..., 'c', '\r', '\n' } @endcode
   * After the call to @c erase, the remaining data is left in the string @c
   * data as follows:
   * @code { 'd', 'e', ... } @endcode
   * This data may be the start of a new line, to be extracted by a subsequent
   * @c async_read_until operation.
  ...
  /// Start an asynchronous operation to read data into a dynamic buffer
  /// sequence until some part of its data matches a regular expression.
  /**
   * This function is used to asynchronously read data into the specified
   * dynamic buffer sequence until the dynamic buffer sequence's get area
   * contains some data that matches a regular expression. The function call
   ...
   * stream's async_read_some function, and is known as a <em>composed
   * operation</em>. If the dynamic buffer sequence's get area already contains
   * data that matches the regular expression, this asynchronous operation
   * completes immediately. The program must ensure that the stream performs no
   * other read operations (such as async_read, async_read_until, the stream's
   * async_read_some function, or any other composed operations that perform
  ...
  /// Start an asynchronous operation to read data into a dynamic buffer
  /// sequence until a function object indicates a match.
  /**
   * This function is used to asynchronously read data into the specified
   * dynamic buffer sequence until a user-defined match condition function
   * object, when applied to the data contained in the dynamic buffer sequence,
   * indicates a successful match. The function call always returns immediately.
   ...
   * This operation is implemented in terms of zero or more calls to the
   * stream's async_read_some function, and is known as a <em>composed
   * operation</em>. If the match condition function object already indicates a
   * match, this asynchronous operation completes immediately. The program must
   * ensure that the stream performs no other read operations (such as
   ...
   * @endcode
   *
   * To asynchronously read data into a @c std::string until a matching
   * character is found:
   * @code class match_char
   * {
   * public:
   ...
   * This operation is implemented in terms of zero or more calls to the
   * stream's async_read_some function, and is known as a <em>composed
   * operation</em>. If the streambuf's get area already contains the delimiter,
   ...
   * leave that data in the streambuf for a subsequent async_read_until
   * operation to examine.
   *
   * @par Example
   * To asynchronously read data into a streambuf until a newline is
   * encountered:
   * @code asio::streambuf b;
   * ...
   * void handler(const asio::error_code& e, std::size_t size)
   ...
   * This operation is implemented in terms of zero or more calls to the
   * stream's async_read_some function, and is known as a <em>composed
   * operation</em>. If the streambuf's get area already contains data that
   * matches the regular expression, this asynchronous operation completes
   * immediately. The program must ensure that the stream performs no other read
   ...
   *
   * @par Examples
   * To asynchronously read data into a streambuf until whitespace is
   * encountered:
   * @code typedef asio::buffers_iterator<
   *     asio::streambuf::const_buffers_type> iterator;
   *
