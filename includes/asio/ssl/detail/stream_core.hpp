...
namespace asio {
  namespace ssl {
    namespace detail {
      ...
      struct stream_core {
        // According to the OpenSSL documentation, this is the buffer size that
        // is sufficient to hold the largest possible TLS record.
        ...
        static asio::deadline_timer::time_type
        expiry(const asio::deadline_timer &timer) {
       ...
       // Timer used for storing queued read operations.
        ...
        // Helper function to get a timer's expiry time.
        static asio::steady_timer::time_point
        expiry(const asio::steady_timer &timer) {
          return timer.expiry();
        }
#endif // defined(ASIO_HAS_BOOST_DATE_TIME)
        ...
        // A buffer that may be used to prepare output intended for the
        // transport.
