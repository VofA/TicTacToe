        ...
        engine::want process(engine &eng,
          ...
          for (;;) {
            ...
            while (iter != end) {
              ...
              // Skip over any buffers which have already been consumed by the
              // engine.
              ...
              // The current buffer may have been partially consumed by the
              // engine on a previous iteration. If so, adjust the buffer to
              // point to the unused portion.
              if (bytes_transferred > accumulated_size)
                buffer = buffer + (bytes_transferred - accumulated_size);

              // Pass the buffer to the engine, and update the bytes transferred
              // to reflect the total number of bytes consumed so far.
