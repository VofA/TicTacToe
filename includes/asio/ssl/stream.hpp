...
namespace asio {
  namespace ssl {
     ...
     * asynchronous operations are performed within the same implicit or
     * explicit strand.
     *
     * @par Example
     ...
     *
     * @par Concepts:
     * AsyncReadStream, AsyncWriteStream, Stream, SyncReadStream,
     * SyncWriteStream.
     */
    ...
    class stream : public stream_base, private noncopyable {
       ...
       * function call will block until handshaking is complete or an error
       * occurs.
       *
       * @param type The type of handshaking to be performed, i.e. as a client
       * or as a server.
       *
       * @throws asio::system_error Thrown on failure.
       */
      ...
      async_handshake(handshake_type type,
        ...
        asio::async_completion<HandshakeHandler, void(asio::error_code)> init(
            handler);

        detail::async_io(next_layer_,
                         core_,
                         detail::handshake_op(type),
       ...
       * the underlying buffers is retained by the caller, which must guarantee
      ...
      /**
       * This function is used to asynchronously shut down SSL on the stream.
      ...
      async_shutdown(ASIO_MOVE_ARG(ShutdownHandler) handler) {
        ...
        asio::async_completion<ShutdownHandler, void(asio::error_code)> init(
            handler);

        detail::async_io(
            next_layer_, core_, detail::shutdown_op(), init.completion_handler);
       ...
       * will block until one or more bytes of data has been written
       ...
       * ); @endcode
       *
       * @note The async_write_some operation may not transmit all of the data
       * to the peer. Consider using the @ref async_write function if you need
       ...
       * @throws asio::system_error Thrown on failure.
       *
       * @note The read_some operation may not read all of the requested number
       * of bytes. Consider using the @ref read function if you need to ensure
       ...
       *
       * @param buffers The buffers into which the data will be read. Although
       * the buffers object may be copied as necessary, ownership of the
       * underlying buffers is retained by the caller, which must guarantee that
       ...
       * number of bytes. Consider using the @ref async_read function if you
      ...
      async_read_some(const MutableBufferSequence &buffers,
        ...
        asio::async_completion<ReadHandler, void(asio::error_code, std::size_t)>
            init(handler);
