...
namespace asio {
   ...
   *
   * Class execution_context implements an extensible, type-safe, polymorphic
   * set of services, indexed by service type.
   *
   ...
   *
   * In a call to @c use_service<Service>(), the type argument chooses a
   * service, making available all members of the named type. If @c Service is
   * not present in an execution_context, an object of type @c Service is
   * created and added to the execution_context. A C++ program can check if an
   * execution_context implements a particular service with the function
   * template @c has_service<Service>().
   *
   ...
   * present, the service_already_exists exception is thrown. If the owner of
   * the service is not the same object as the execution_context parameter, the
   ...
   *
   * All service implementations have execution_context::service as a public
   * base class. Custom services may be implemented by deriving from this class
   * and then added to an execution_context using the facilities described
   * above.
   *
   ...
   * destructor calls @c shutdown() and @c destroy() to destroy all pending
  ...
  class execution_context : private noncopyable {
     ...
     * object lifetime. Otherwise, services are visited in order of the
     * beginning of service object lifetime.
     */
     ...
     * given service type. If there is no existing implementation of the
     * service, then the execution_context will create a new instance of the
     * service.
     *
     ...
     * This function is used to locate a service object that corresponds to the
     * given service type. If there is no existing implementation of the
     * service, then the io_context will create a new instance of the service.
     *
     ...
     * Ownership of the service interface is not transferred to the caller.
     *
     * @note This overload is preserved for backwards compatibility with
     * services that inherit from io_context::service.
     */
    template <typename Service>
...
#undef ASIO_PRIVATE_MAKE_SERVICE_DEF
     ...
     * @param e The execution_context object that owns the service.
     *
     * @param svc The service object. On success, ownership of the service
     * object is transferred to the execution_context. When the
     * execution_context object is destroyed, it will destroy the service object
     * by performing: @code delete static_cast<execution_context::service*>(svc)
     * @endcode
     *
     * @throws asio::service_already_exists Thrown if a service of the
     * given type is already present in the execution_context.
    ...
    /**
     * This function is used to determine whether the execution_context contains
     * a service object corresponding to the given service type.
     *
     * @param e The execution_context object that owns the service.
     *
