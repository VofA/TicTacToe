...
namespace asio {
  namespace posix {
     ...
     */
    class descriptor : ASIO_SVC_ACCESS basic_io_object<ASIO_SVC_T>,
       ...
       * occur.
       *
       * @note Following the move, the moved-from object is in the same state as
       * if constructed using the @c descriptor(io_context&) constructor.
       */
      ...
      /// (Deprecated: Use get_executor().) Get the io_context associated with
      /// the object.
      /**
       ...
       *
       * @return A reference to the lowest layer in the stack of layers.
       ...
       * of layers. Since a descriptor cannot contain any further layers, it
       * simply returns a reference to itself.
       *
      ...
      /*
       * This function opens the descriptor to hold an existing native
       * descriptor.
       *
       * @param native_descriptor A native descriptor.
       *
       ...
       * write operations will be cancelled immediately, and will complete with
       * the asio::error::operation_aborted error.
       *
       ...
       * the asio::error::operation_aborted error.
       *
       * @param ec Set to indicate what error occurred, if any. Note that, even
       * if the function indicates an error, the underlying descriptor is
       * closed.
       */
      ...
      /**
       * This function may be used to obtain the underlying representation of
       * the descriptor. This is intended to allow access to native descriptor
      ...
      /**
       * This function may be used to obtain the underlying representation of
       * the descriptor. After calling this function, @c is_open() returns
       ...
       * immediately, and the handlers for cancelled operations will be passed
       * the asio::error::operation_aborted error.
       */
      ...
      /**
       * This function causes all outstanding asynchronous read or write
      ...
      /**
       * This function is used to execute an IO control command on the
       * descriptor.
       *
       * @param command The IO control command to be performed on the
       * descriptor.
       *
       * @throws asio::system_error Thrown on failure.
       *
      ...
      /**
       * @returns @c true if the descriptor's synchronous operations will fail
       * with asio::error::would_block if they are unable to perform the
       * requested operation immediately. If @c false, synchronous operations
       * will block until complete.
       *
       * @note The non-blocking mode has no effect on the behaviour of
       * asynchronous operations. Asynchronous operations will never fail with
       * the error asio::error::would_block.
       */
      ...
      /**
       * @param mode If @c true, the descriptor's synchronous operations will
       * fail with asio::error::would_block if they are unable to perform the
       * requested operation immediately. If @c false, synchronous operations
       * will block until complete.
       *
       * @throws asio::system_error Thrown on failure.
      ...
      /**
       * This function is used to retrieve the non-blocking mode of the
       * underlying native descriptor. This mode has no effect on the behaviour
       * of the descriptor object's synchronous operations.
       *
       * @returns @c true if the underlying descriptor is in non-blocking mode
       * and direct system calls may fail with asio::error::would_block (or the
       * equivalent system error).
       *
       * @note The current non-blocking mode is cached by the descriptor object.
       * Consequently, the return value may be incorrect if the non-blocking
       * mode was set directly on the native descriptor.
       */
       ...
       *
       * @param mode If @c true, the underlying descriptor is put into
       * non-blocking mode and direct system calls may fail with
       * asio::error::would_block (or the equivalent system error).
       *
       ...
       * asio::error::would_block (or the equivalent system error).
       *
       * @param ec Set to indicate what error occurred, if any. If the @c mode
       * is
       * @c false, but the current value of @c non_blocking() is @c true, this
       * function fails with asio::error::invalid_argument, as the
       * combination does not make sense.
      ...
      /// Asynchronously wait for the descriptor to become ready to read, ready
       ...
       * @param w Specifies the desired descriptor state.
       *
       * @param handler The handler to be called when the wait operation
