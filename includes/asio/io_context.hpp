...
namespace asio {
   ...
   * run(), run_one(), run_for(), run_until(), poll() or poll_one() calls
   * results in undefined behaviour. The notify_fork() function should not be
   * called while any io_context function, or any function on an I/O object that
   * is associated with the io_context, is being called in another thread.
   *
   * @par Concepts:
   * Dispatcher.
   *
   ...
   * run_until(), poll() or poll_one() call may be restarted @em without the
   * need for an intervening call to restart(). This allows the thread to rejoin
   * the io_context object's thread pool without impacting any other threads in
   * the pool.
   *
   * For example:
   *
   ...
   * from returning when there is no more work to do. For example, the
   * io_context may be being run in a background thread that is launched prior
   * to the application's asynchronous operations. The run() call may be kept
   * running by creating an object of type
  ...
  class io_context : public execution_context {
     ...
     * operations:
     *
     * @li For each service object @c svc in the io_context set, in reverse
     * order of the beginning of service object lifetime, performs
     * @c svc->shutdown().
     *
     ...
     *
     * @li For each service object @c svc in the io_context set, in reverse
     * order of the beginning of service object lifetime, performs <tt>delete
     * static_cast<io_context::service*>(svc)</tt>.
     *
     ...
     * run(), run_one(), poll() or poll_one() will return immediately unless
     * there is a prior call to restart().
     *
     ...
     * the same io_context object may introduce the potential for deadlock. It
     * is the caller's reponsibility to avoid this.
     *
    ...
    /// duration.
    /**
     * The run_for() function blocks until all work has finished and there are
     ...
     * until the io_context has been stopped, or until the specified duration
     * has elapsed.
     *
     * @param rel_time The duration for which the call may block.
     *
    ...
    /**
     * The run_one_until() function blocks until one handler has been
     * dispatched, until the io_context has been stopped, or until the specified
    ...
    /// handlers.
    /**
     * The poll() function runs handlers that are ready to run, without
     * blocking, until the io_context has been stopped or there are no more
     * ready handlers.
     *
     * @return The number of handlers that were executed.
     */
     ...
     * to stop. All invocations of its run() or run_one() member functions
     * should return as soon as possible. Subsequent calls to run(), run_one(),
     * poll() or poll_one() will return immediately until restart() is called.
     */
    ASIO_DECL void stop();
     ...
     * a previous invocation of these functions returned due to the io_context
     ...
     *
     * The io_context guarantees that the handler will only be called in a
     * thread in which the run(), run_one(), poll() or poll_one() member
     * functions is currently being invoked. The handler may be executed inside
     ...
     * The io_context guarantees that the handler will only be called in a
     * thread in which the run(), run_one(), poll() or poll_one() member
     * functions is currently being invoked.
     *
     * @param handler The handler to be called. The io_context will make
     * a copy of the handler object as required. The function signature of the
     * handler must be: @code void handler(); @endcode
     ...
     *
     * @return A function object that, when invoked, passes the wrapped handler
     * to the io_context object's dispatch function. Given a function object
     * with the signature:
  ...
  class io_context::executor_type {
     ...
     * scheduling the function object until the current thread returns control
     * to the pool.
     *
     * @param f The function object to be called. The executor will make a copy
     * of the handler object as required. The function signature of the function
    ...
    /**
     * @return @c true if the current thread is running the io_context.
   ...
   *
   * The work class is copy-constructible so that it may be used as a data
   * member in a handler class. It is not assignable.
   */
  class io_context::work {
    public:
    /// Constructor notifies the io_context that work is starting.
    /**
     * The constructor is used to inform the io_context that some work has
     * begun. This ensures that the io_context object's run() function will not
     * exit while the work is underway.
     */
    ...
    /// (Deprecated: Use get_io_context().) Get the io_context associated with
    /// the work.
  ...
  namespace detail {
    ...
    class service_base : public asio::io_context::service {
      public:
