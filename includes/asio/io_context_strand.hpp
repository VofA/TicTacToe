...
namespace asio {
   ...
   * happens-before the other. Therefore none of the above conditions are met
   * and no ordering guarantee is made.
   *
   * @note The implementation makes no guarantee that handlers posted or
   * dispatched through different @c strand objects will be invoked
   * concurrently.
   *
   * @par Thread Safety
  ...
  class io_context::strand {
    ...
    explicit strand(asio::io_context &io_context)
        : service_(
              asio::use_service<asio::detail::strand_service>(io_context)) {
     ...
     * underlying io_context. The io_context guarantees that the handler will
     * only be called in a thread in which the io_context's run member function
     ...
     * without allowing the strand to call the handler from inside this
     * function.
     *
     * The strand object guarantees that handlers posted or dispatched through
     * the strand will not be executed concurrently. The strand's guarantee is
     * in addition to the guarantee provided by the underlying io_context. The
     ...
     * to the strand's dispatch function. Given a function object with the
     * signature:
    ...
    wrap(Handler handler) {
      return detail::wrapped_handler<io_context::strand,
                                     Handler,
