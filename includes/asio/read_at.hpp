...
namespace asio {
   ...
   *
   * @param d The device from which the data is to be read. The type must
   * support the SyncRandomAccessReadDevice concept.
   *
   ...
   * the next call to the device's read_some_at function.
   *
   * @returns The number of bytes transferred.
   *
   * @throws asio::system_error Thrown on failure.
   ...
   *
   * This operation is implemented in terms of zero or more calls to the
   * device's async_read_some_at function.
   *
   * @param d The device from which the data is to be read. The type must
   * support the AsyncRandomAccessReadDevice concept.
   *
   * @param offset The offset at which the data will be read.
   *
   * @param buffers One or more buffers into which the data will be read. The
   ...
   * A return value of 0 indicates that the read operation is complete. A
   * non-zero return value indicates the maximum number of bytes to be read on
   * the next call to the device's async_read_some_at function.
   *
   * @param handler The handler to be called when the read operation completes.
   * Copies will be made of the handler as required. The function signature of
   * the handler must be:
