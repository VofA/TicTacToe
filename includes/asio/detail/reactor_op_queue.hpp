...
namespace asio {
  namespace detail {
    ...
    class reactor_op_queue : private noncopyable {
      public:
      ...
      // the reactor's event demultiplexing function may need to be interrupted
      // and restarted.
      ...
      // Determine whether there are any operations associated with the
      // descriptor.
      bool has_operation(Descriptor descriptor) const {
        return operations_.find(descriptor) != operations_.end();
      }

      // Perform the operations corresponding to the descriptor identified by
      // the supplied iterator. Returns true if there are still unfinished
      // operations queued for the descriptor.
