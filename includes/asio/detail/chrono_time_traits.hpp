...
namespace asio {
  namespace detail {
    ...
    struct chrono_time_traits {
      ...
      // Implement just enough of the posix_time::time_duration interface to
      // supply what the timer_queue requires.
      class posix_time_duration {
        public:
        explicit posix_time_duration(const duration_type &d) : d_(d) {
        }
        ...
        int64_t duration_cast() const {
          const int64_t num1 =
              period_type::num / gcd<period_type::num, Num>::value;
          const int64_t num2 = Num / gcd<period_type::num, Num>::value;

          const int64_t den1 =
              period_type::den / gcd<period_type::den, Den>::value;
