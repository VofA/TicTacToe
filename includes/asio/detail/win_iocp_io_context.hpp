...
namespace asio {
  namespace detail {
    ...
    class win_iocp_io_context
      ...
      // Assumes that work_started() has not yet been called for the operation.
      ...
      // Request invocation of the given operation and return immediately.
      // Assumes that work_started() was previously called for the operation.
      ASIO_DECL void post_deferred_completion(win_iocp_operation *op);

      // Request invocation of the given operation and return immediately.
      // Assumes that work_started() was previously called for the operations.
      ASIO_DECL void
      post_deferred_completions(op_queue<win_iocp_operation> &ops);

      // Request invocation of the given operation using the thread-private
      ...
      // Request invocation of the given operation using the thread-private
      // queue and return immediately. Assumes that work_started() was
      // previously called for the operation.
      ...
      // immediately. The caller must have already called work_started() prior
      ...
      template <typename Time_Traits>
      void
      schedule_timer(timer_queue<Time_Traits> &queue,
      ...
      template <typename Time_Traits>
      std::size_t
      cancel_timer(timer_queue<Time_Traits> &queue,
                   typename timer_queue<Time_Traits>::per_timer_data &timer,
      ...
      // avoid exhausting this resouce we limit the number of outstanding
      // events.
      ...
      enum {
        ...
        // GetQueuedCompletionStatus can appear stuck even though there are
        // events waiting on the queue. Using a timeout helps to work around the
        // issue.
        ...
        // Completion key value to indicate that an operation has posted with
        // the original last_error and bytes_transferred values stored in the
        // fields of the OVERLAPPED structure.
      ...
      // Mutex for protecting access to the timer queues and completed
      // operations.
