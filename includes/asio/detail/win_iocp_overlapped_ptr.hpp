...
namespace asio {
  namespace detail {
    ...
    // Wraps a handler to create an OVERLAPPED object for use with overlapped
    // I/O.
    class win_iocp_overlapped_ptr : private noncopyable {
      public:
      // Construct an empty win_iocp_overlapped_ptr.
      win_iocp_overlapped_ptr() : ptr_(0), iocp_service_(0) {
      }

      // Construct an win_iocp_overlapped_ptr to contain the specified handler.
      template <typename Handler>
      explicit win_iocp_overlapped_ptr(asio::io_context &io_context,
                                       ASIO_MOVE_ARG(Handler) handler)
          : ptr_(0), iocp_service_(0) {
      ...
      void reset(asio::io_context &io_context, Handler handler) {
        ...
        ASIO_HANDLER_CREATION((io_context,
                               *p.p,
                               "io_context",
                               &io_context.impl_,
                               0,
      ...
      void complete(const asio::error_code &ec, std::size_t bytes_transferred) {
        if (ptr_) {
          iocp_service_->on_completion(
              ptr_, ec, static_cast<DWORD>(bytes_transferred));
          ptr_          = 0;
          iocp_service_ = 0;
        }
      }
