  ...
  namespace detail {
    ...
    class win_iocp_socket_service_base {
      ...
      struct base_implementation_type {
        ...
        // We use a shared pointer as a cancellation token here to work around
        // the broken Windows support for cancellation. MSDN says that when you
        // call closesocket any outstanding WSARecv or WSASend operations will
        // complete with the error ERROR_OPERATION_ABORTED. In practice they
        // complete with ERROR_NETNAME_DELETED, which means you can't tell the
        // difference between a local cancellation and the socket being
      ...
      // Constructor.
      ASIO_DECL win_iocp_socket_service_base(asio::io_context &io_context);
      ...
      ASIO_DECL void
      base_move_assign(base_implementation_type &impl,
      ...
      ASIO_DECL socket_type release(base_implementation_type &impl,
                                    asio::error_code &ec);
      ...
      // Place the socket into the state where it will listen for new
      // connections.
      ...
      asio::error_code native_non_blocking(base_implementation_type &impl,
        ...
        socket_ops::set_internal_non_blocking(
            impl.socket_, impl.state_, mode, ec);
        return ec;
      }
      ...
      void async_wait(base_implementation_type &impl,
        ...
        typename op::ptr p = {
            asio::detail::addressof(handler), op::ptr::allocate(handler), 0};
        p.p = new (p.v) op(impl.cancel_token_, handler);

        ASIO_HANDLER_CREATION(
            (io_context_, *p.p, "socket", &impl, impl.socket_, "async_wait"));
      ...
      size_t send(base_implementation_type &impl,
                  ...
                  asio::error_code &ec) {
        buffer_sequence_adapter<asio::const_buffer, ConstBufferSequence> bufs(
            buffers);

        return socket_ops::sync_send(impl.socket_,
                                     impl.state_,
                                     bufs.buffers(),
                                     bufs.count(),
                                     flags,
      ...
      void async_send(base_implementation_type &impl,
        ...
        ASIO_HANDLER_CREATION(
            (io_context_, *p.p, "socket", &impl, impl.socket_, "async_send"));
        ...
        start_send_op(impl,
                      ...
                      bufs.count(),
                      flags,
                      (impl.state_ & socket_ops::stream_oriented) != 0 &&
                          bufs.all_empty(),
                      p.p);
        p.v = p.p = 0;
      }
      ...
      void async_send(base_implementation_type &impl,
        ...
        ASIO_HANDLER_CREATION((io_context_,
                               *p.p,
                               "socket",
                               &impl,
                               impl.socket_,
      ...
      size_t receive(base_implementation_type &impl,
                     ...
                     socket_base::message_flags flags,
                     asio::error_code &ec) {
        buffer_sequence_adapter<asio::mutable_buffer, MutableBufferSequence>
            bufs(buffers);

        return socket_ops::sync_recv(impl.socket_,
                                     impl.state_,
                                     bufs.buffers(),
                                     bufs.count(),
                                     flags,
      ...
      void async_receive(base_implementation_type &impl,
        ...
        ASIO_HANDLER_CREATION((
            io_context_, *p.p, "socket", &impl, impl.socket_, "async_receive"));
      ...
      size_t receive_with_flags(base_implementation_type &impl,
        ...
        return socket_ops::sync_recvmsg(impl.socket_,
                                        impl.state_,
                                        bufs.buffers(),
                                        bufs.count(),
                                        in_flags,
      ...
      size_t receive_with_flags(base_implementation_type &impl,
        ...
        // Clear out_flags, since we cannot give it any other sensible value
        // when performing a null_buffers operation.
        out_flags = 0;
      ...
      void async_receive_with_flags(base_implementation_type &impl,
        ...
        start_receive_op(
            impl, bufs.buffers(), bufs.count(), in_flags, false, p.p);
        p.v = p.p = 0;
      }
      ...
      // Helper function to start an asynchronous null_buffers receive
      // operation.
      ASIO_DECL void
      start_null_buffers_receive_op(base_implementation_type &impl,
                                    socket_base::message_flags flags,
                                    reactor_op *op);
      ...
      ASIO_DECL void start_connect_op(base_implementation_type &impl,
                                      int family,
                                      int type,
                                      const socket_addr_type *remote_addr,
      ...
      ASIO_DECL void
      update_cancellation_thread_id(base_implementation_type &impl);

      // Helper function to get the reactor. If no reactor has been created yet,
      // a new one is obtained from the io_context and a pointer to it is cached
      // in this service.
      ASIO_DECL select_reactor &get_reactor();

      // The type of a ConnectEx function pointer, as old SDKs may not provide
      // it.
      ...
      // Helper function to get the ConnectEx pointer. If no ConnectEx pointer
      // has been obtained yet, one is obtained using WSAIoctl and the pointer
      // is cached. Returns a null pointer if ConnectEx is not available.
      ASIO_DECL connect_ex_fn get_connect_ex(base_implementation_type &impl,
                                             int type);

      // The type of a NtSetInformationFile function pointer.
      typedef LONG(NTAPI *nt_set_info_fn)(
          HANDLE, ULONG_PTR *, void *, ULONG, ULONG);

      // Helper function to get the NtSetInformationFile function pointer. If no
      // NtSetInformationFile pointer has been obtained yet, one is obtained
      // using GetProcAddress and the pointer is cached. Returns a null pointer
      // if NtSetInformationFile is not available.
      ASIO_DECL nt_set_info_fn get_nt_set_info();

      // Helper function to emulate InterlockedCompareExchangePointer
      // functionality for:
      // - very old Platform SDKs; and
      // - platform SDKs where MSVC's /Wp64 option causes spurious warnings.
      ASIO_DECL void *
      interlocked_compare_exchange_pointer(void **dest, void *exch, void *cmp);

      // Helper function to emulate InterlockedExchangePointer functionality
      // for:
      // - very old Platform SDKs; and
      // - platform SDKs where MSVC's /Wp64 option causes spurious warnings.
      ...
      // The IOCP service used for running asynchronous operations and
      // dispatching handlers.
      win_iocp_io_context &iocp_service_;

      // The reactor used for performing connect operations. This object is
