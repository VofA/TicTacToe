...
namespace asio {
  namespace detail {
    ...
    auto zero_arg_copyable_handler_test(Handler h, void *)
        -> decltype(sizeof(Handler(static_cast<const Handler &>(h))),
                    ((h)()),
                    char(0));
    ...
    auto one_arg_handler_test(Handler h, Arg1 *a1)
        -> decltype(sizeof(Handler(ASIO_MOVE_CAST(Handler)(h))),
                    ((h)(*a1)),
                    char(0));
...
#define ASIO_LEGACY_COMPLETION_HANDLER_CHECK(handler_type, handler)       \
  ...
  ASIO_HANDLER_TYPE_REQUIREMENTS_ASSERT(                                  \
      sizeof(asio::detail::zero_arg_copyable_handler_test(                \
          asio::detail::clvref<asio_true_handler_type>(), 0)) == 1,       \
      "CompletionHandler type requirements not met")                      \
                                                                          \
  typedef asio::detail::handler_type_requirements<                        \
      sizeof(asio::detail::argbyv(                                        \
          asio::detail::clvref<asio_true_handler_type>())) +              \
      sizeof(asio::detail::lvref<asio_true_handler_type>()(), char(0))>   \
      ASIO_UNUSED_TYPEDEF

#define ASIO_READ_HANDLER_CHECK(handler_type, handler)                         \
                                                                               \
  typedef ASIO_HANDLER_TYPE(handler_type, void(asio::error_code, std::size_t)) \
      asio_true_handler_type;                                                  \
                                                                               \
  ASIO_HANDLER_TYPE_REQUIREMENTS_ASSERT(                                       \
      sizeof(asio::detail::two_arg_handler_test(                               \
          asio::detail::rvref<asio_true_handler_type>(),                       \
  ...
  typedef asio::detail::handler_type_requirements<                             \
      sizeof(asio::detail::argbyv(                                             \
          asio::detail::rvref<asio_true_handler_type>())) +                    \
      sizeof(asio::detail::lvref<asio_true_handler_type>()(                    \
...
#define ASIO_ACCEPT_HANDLER_CHECK(handler_type, handler)          \
                                                                  \
  typedef ASIO_HANDLER_TYPE(handler_type, void(asio::error_code)) \
      asio_true_handler_type;                                     \
                                                                  \
  ASIO_HANDLER_TYPE_REQUIREMENTS_ASSERT(                          \
...
#define ASIO_MOVE_ACCEPT_HANDLER_CHECK(handler_type, handler, socket_type)     \
                                                                               \
  typedef ASIO_HANDLER_TYPE(handler_type, void(asio::error_code, socket_type)) \
      asio_true_handler_type;                                                  \
                                                                               \
  ASIO_HANDLER_TYPE_REQUIREMENTS_ASSERT(                                       \
...
#define ASIO_ITERATOR_CONNECT_HANDLER_CHECK(handler_type, handler, iter_type) \
                                                                              \
  typedef ASIO_HANDLER_TYPE(handler_type, void(asio::error_code, iter_type))  \
      asio_true_handler_type;                                                 \
                                                                              \
  ASIO_HANDLER_TYPE_REQUIREMENTS_ASSERT(                                      \
      sizeof(asio::detail::two_arg_handler_test(                              \
...
#define ASIO_RESOLVE_HANDLER_CHECK(handler_type, handler, range_type)         \
                                                                              \
  typedef ASIO_HANDLER_TYPE(handler_type, void(asio::error_code, range_type)) \
      asio_true_handler_type;                                                 \
                                                                              \
  ASIO_HANDLER_TYPE_REQUIREMENTS_ASSERT(                                      \
      sizeof(asio::detail::two_arg_handler_test(                              \
...
#define ASIO_SIGNAL_HANDLER_CHECK(handler_type, handler)               \
                                                                       \
  typedef ASIO_HANDLER_TYPE(handler_type, void(asio::error_code, int)) \
      asio_true_handler_type;                                          \
                                                                       \
  ASIO_HANDLER_TYPE_REQUIREMENTS_ASSERT(                               \
      sizeof(asio::detail::two_arg_handler_test(                       \
...
#define ASIO_RANGE_CONNECT_HANDLER_CHECK(handler_type, handler, iter_type) \
  typedef int ASIO_UNUSED_TYPEDEF
