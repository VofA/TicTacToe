...
namespace asio {
  ...
  namespace detail {
    ...
    class service_registry : private noncopyable {
      ...
      // Get the service object corresponding to the specified service type.
      // Will create a new service object automatically if no such object
      // already exists. Ownership of the service object is not transferred to
      // the caller.
      ...
      // Will create a new service object automatically if no such object
      // already exists. Ownership of the service object is not transferred to
      // the caller. This overload is used for backwards compatibility with
      ...
      init_key_from_id(execution_context::service::key &key,
      ...
      ASIO_DECL static bool
      keys_match(const execution_context::service::key &key1,
      ...
      // exists. Ownership of the service object is not transferred to the
      // caller.
      ASIO_DECL execution_context::service *
      do_use_service(const execution_context::service::key &key,
                     factory_type factory,
                     void *owner);

      // Add a service object. Throws on error, in which case ownership of the
      // object is retained by the caller.
      ASIO_DECL void do_add_service(const execution_context::service::key &key,
                                    execution_context::service *new_service);

      // Check whether a service object with the specified key already exists.
      ASIO_DECL bool
      do_has_service(const execution_context::service::key &key) const;
