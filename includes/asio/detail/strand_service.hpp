...
namespace asio {
  namespace detail {
    ...
    class strand_service : public asio::detail::service_base<strand_service> {
      ...
      // Request the io_context to invoke the given handler and return
      // immediately.
      template <typename Handler>
      ...
      ASIO_DECL bool
      running_in_this_thread(const implementation_type &impl) const;

      private:
      // Helper function to dispatch a handler. Returns true if the handler
      ...
      ASIO_DECL void
      do_post(implementation_type &impl, operation *op, bool is_continuation);
