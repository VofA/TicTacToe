...
namespace asio {
  namespace detail {
    ...
    class dev_poll_reactor
      ...
      ASIO_DECL int
      register_internal_descriptor(int op_type,
                                   socket_type descriptor,
                                   per_descriptor_data &descriptor_data,
                                   reactor_op *op);

      // Move descriptor registration from one descriptor_data object to
      // another.
      ASIO_DECL void
      move_descriptor(socket_type descriptor,
      ...
      // Cancel any operations that are running against the descriptor and
      // remove its registration from the reactor. The reactor resources
      // associated with the descriptor must be released by calling
      ...
      // cleanup_descriptor_data.
      ASIO_DECL void deregister_internal_descriptor(socket_type descriptor,
                                                    per_descriptor_data &);
      ...
      // Cancel the timer operations associated with the given token. Returns
      // the number of operations that have been posted or dispatched.
      template <typename Time_Traits>
      std::size_t
      ...
      template <typename Time_Traits>
      void
      move_timer(timer_queue<Time_Traits> &queue,
                 typename timer_queue<Time_Traits>::per_timer_data &target,
                 typename timer_queue<Time_Traits>::per_timer_data &source);

      // Run /dev/poll once until interrupted or events are ready to be
      // dispatched.
      ...
      // Cancel all operations associated with the given descriptor. The
      // do_cancel function of the handler objects will be invoked. This
      // function does not acquire the dev_poll_reactor's mutex.
