...
namespace asio {
  namespace detail {
    ...
    class winsock_init_base {
      ...
      // used to ensure that the values are zero-initialised prior to any code
      // being run.
      struct data {
        long init_count_;
        long result_;
      ...
      startup(data &d, unsigned char major, unsigned char minor);
    ...
    class winsock_init : private winsock_init_base {
      ...
      // initialisation and cleanup. This may be required in the case of a DLL,
      // for example, where it is not safe to initialise Winsock from global
      // object constructors.
      //
      // To prevent asio from initialising Winsock, the object must be
      // constructed before any Asio's own global objects. With MSVC, this may
      // be accomplished by adding the following code to the DLL:
      //
      //   #pragma warning(push)
