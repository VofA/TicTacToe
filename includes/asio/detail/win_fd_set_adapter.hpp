...
namespace asio {
  namespace detail {
    ...
    class win_fd_set_adapter : noncopyable {
      ...
      win_fd_set_adapter()
          : capacity_(default_fd_set_size), max_descriptor_(invalid_socket) {
        fd_set_ = static_cast<win_fd_set *>(
            ::operator new(sizeof(win_fd_set) - sizeof(SOCKET) +
      ...
      void set(reactor_op_queue<socket_type> &operations,
               op_queue<operation> &) {
      ...
      // structure, but without being dependent on the value of FD_SETSIZE. We
      // use the "struct hack" to allow the number of descriptors to be varied
      // at runtime.
      struct win_fd_set {
        u_int fd_count;
        SOCKET fd_array[1];
