  ...
  namespace detail {
    ...
    class strand_executor_service
      ...
      class strand_impl {
        ...
        // Indicates that the strand has been shut down and will accept no
        // further handlers.
        bool shutdown_;

        // The handlers that are waiting on the strand but should not be run
        // until after the next time the strand is scheduled. This queue must
        // only be modified while the mutex is locked.
        op_queue<scheduler_operation> waiting_queue_;

        // The handlers that are ready to be run. Logically speaking, these are
        // the handlers that hold the strand's lock. The ready queue is only
        // modified from within the strand and so may be accessed without
        // locking the mutex.
      ...
      static void dispatch(const implementation_type &impl,
                           Executor &ex,
      ...
      template <typename Executor, typename Function, typename Allocator>
      static void post(const implementation_type &impl,
                       Executor &ex,
                       ASIO_MOVE_ARG(Function) function,
                       const Allocator &a);
      ...
      // Request invocation of the given function and return immediately.
      template <typename Executor, typename Function, typename Allocator>
      static void defer(const implementation_type &impl,
                        Executor &ex,
                        ASIO_MOVE_ARG(Function) function,
                        const Allocator &a);
      ...
      running_in_this_thread(const implementation_type &impl);
