...
namespace asio {
  namespace detail {
    ...
    class reactive_serial_port_service
      ...
      typedef reactive_descriptor_service::native_handle_type
          native_handle_type;

      // The implementation type of the serial port.
      typedef reactive_descriptor_service::implementation_type
          implementation_type;

      ASIO_DECL reactive_serial_port_service(asio::io_context &io_context);
      ...
      void move_assign(implementation_type &impl,
                       ...
                       implementation_type &other_impl) {
        descriptor_service_.move_assign(
            impl, other_service.descriptor_service_, other_impl);
      }
      ...
      asio::error_code set_option(implementation_type &impl,
        ...
        return do_set_option(impl,
                             &reactive_serial_port_service::store_option<
      ...
      asio::error_code get_option(const implementation_type &impl,
        ...
        return do_get_option(impl,
                             &reactive_serial_port_service::load_option<
      ...
      asio::error_code send_break(implementation_type &impl,
                                  asio::error_code &ec) {
        errno = 0;
        descriptor_ops::error_wrapper(
            ::tcsendbreak(descriptor_service_.native_handle(impl), 0), ec);
        return ec;
      }
      ...
      // Start an asynchronous write. The data being written must be valid for
      // the lifetime of the asynchronous operation.
      ...
      // Start an asynchronous read. The buffer for the data being received must
      ...
      typedef asio::error_code (*store_function_type)(const void *,
                                                      termios &,
                                                      asio::error_code &);
      ...
      store_option(const void *option, termios &storage, asio::error_code &ec) {
        static_cast<const SettableSerialPortOption *>(option)->store(storage,
                                                                     ec);
        return ec;
      }

      // Helper function to set a serial port option.
      ASIO_DECL asio::error_code do_set_option(implementation_type &impl,
                                               store_function_type store,
                                               const void *option,
                                               asio::error_code &ec);

      // Function pointer type for loading a serial port option.
      typedef asio::error_code (*load_function_type)(void *,
                                                     const termios &,
                                                     asio::error_code &);
      ...
      template <typename GettableSerialPortOption>
      static asio::error_code
      load_option(void *option, const termios &storage, asio::error_code &ec) {
        static_cast<GettableSerialPortOption *>(option)->load(storage, ec);
        return ec;
      }

      // Helper function to get a serial port option.
      ASIO_DECL asio::error_code do_get_option(const implementation_type &impl,
