...
namespace asio {
  namespace detail {
    ...
    class resolver_service : public service_base<resolver_service<Protocol>>,
                             public resolver_service_base {
      public:
      // The implementation type of the resolver. A cancellation token is used
      // to indicate to the background thread that the operation has been
      // cancelled.
      ...
      results_type resolve(implementation_type &,
        ...
        socket_ops::getaddrinfo(query.host_name().c_str(),
                                query.service_name().c_str(),
                                query.hints(),
                                &address_info,
                                ec);
        auto_addrinfo auto_address_info(address_info);

        return ec ? results_type()
                  : results_type::create(
      ...
      results_type resolve(implementation_type &,
        ...
        socket_ops::sync_getnameinfo(endpoint.data(),
                                     endpoint.size(),
                                     host_name,
                                     NI_MAXHOST,
                                     service_name,
        ...
        return ec ? results_type()
                  : results_type::create(endpoint, host_name, service_name);
      }
