    ...
    class winrt_async_manager
        ...
        action->Completed = ref new AsyncActionCompletedHandler(
            [promise](IAsyncAction ^ action, AsyncStatus status) {
              switch (status) {
                ...
                default:
                  asio::error_code ec(action->ErrorCode.Value,
      ...
      TResult sync(Windows::Foundation::IAsyncOperation<TResult> ^ operation,
        ...
        operation->Completed = ref new AsyncOperationCompletedHandler<TResult>(
            [promise](IAsyncOperation<TResult> ^ operation,
                      AsyncStatus status) {
              switch (status) {
                case AsyncStatus::Canceled:
                  promise->set_value(asio::error::operation_aborted);
                ...
                case AsyncStatus::Completed:
                default:
                  asio::error_code ec(operation->ErrorCode.Value,
                                      asio::system_category());
                  promise->set_value(ec);
                  break;
              }
      ...
      TResult sync(
        ...
        operation->Completed =
            ref new AsyncOperationWithProgressCompletedHandler<TResult,
                                                               TProgress>(
                [promise](IAsyncOperationWithProgress<TResult, TProgress> ^
                              operation,
                          AsyncStatus status) {
                  switch (status) {
                    case AsyncStatus::Canceled:
        ...
        auto on_completed = ref new AsyncActionCompletedHandler(
            [this, handler](IAsyncAction ^ action, AsyncStatus status) {
              switch (status) {
                ...
                case AsyncStatus::Error:
                default:
                  handler->ec_ = asio::error_code(action->ErrorCode.Value,
                                                  asio::system_category());
                  break;
              }
      ...
      void async(Windows::Foundation::IAsyncOperation<TResult> ^ operation,
        ...
        auto on_completed = ref new AsyncOperationCompletedHandler<TResult>(
            [this, handler](IAsyncOperation<TResult> ^ operation,
                            AsyncStatus status) {
              switch (status) {
                case AsyncStatus::Canceled:
                  handler->ec_ = asio::error::operation_aborted;
                ...
                case AsyncStatus::Error:
                default:
                  handler->ec_ = asio::error_code(operation->ErrorCode.Value,
                                                  asio::system_category());
                  break;
              }
              io_context_.post_deferred_completion(handler);
                  ...
                  switch (status) {
                    ...
                    default:
                      handler->ec_ = asio::error_code(
                          operation->ErrorCode.Value, asio::system_category());
                      break;
                  }
                  io_context_.post_deferred_completion(handler);
                  if (--outstanding_ops_ == 0)
