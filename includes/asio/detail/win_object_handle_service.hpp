...
namespace asio {
  namespace detail {
    ...
    class win_object_handle_service
      ...
      // Constructor.
      ASIO_DECL win_object_handle_service(asio::io_context &io_context);
      ...
      ASIO_DECL void register_wait_callback(implementation_type &impl,
                                            mutex::scoped_lock &lock);

      // Callback function invoked when the registered wait completes.
      static ASIO_DECL VOID CALLBACK wait_callback(PVOID param,
                                                   BOOLEAN timeout);
