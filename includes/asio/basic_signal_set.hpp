...
namespace asio {
  ...
  class basic_signal_set : public basic_io_object<SignalSetService> {
    ...
    /**
     * This constructor creates a signal set without registering for any
     * signals.
     *
     ...
     */
    basic_signal_set(asio::io_context &io_context,
                     int signal_number_1,
                     int signal_number_2)
        : basic_io_object<SignalSetService>(io_context) {
      asio::error_code ec;
    ...
    /**
     * This function adds the specified signal to the set. It has no effect if
     * the signal is already in the set.
     *
    ...
    async_wait(ASIO_MOVE_ARG(SignalHandler) handler) {
      ...
      return this->get_service().async_wait(
          this->get_implementation(), ASIO_MOVE_CAST(SignalHandler)(handler));
    }
  };
