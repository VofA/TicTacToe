...
#define ASIO_SVC_T asio::detail::winrt_resolver_service<InternetProtocol>
#else
#include "asio/detail/resolver_service.hpp"
#define ASIO_SVC_T asio::detail::resolver_service<InternetProtocol>
#endif
...
namespace asio {
  namespace ip {
     ...
     */
    template <typename InternetProtocol ASIO_SVC_TPARAM_DEF1(
        = resolver_service<InternetProtocol>)>
    class basic_resolver : ASIO_SVC_ACCESS basic_io_object<ASIO_SVC_T>,
       ...
       * if constructed using the @c basic_resolver(io_context&) constructor.
       */
       ...
       * Cancels any outstanding asynchronous operations associated with the
       * target object.
       *
       * @param other The other basic_resolver object from which the move will
       * occur.
       ...
       * asynchronous wait operations associated with the resolver as if by
       * calling
       * @c cancel.
       */
      ~basic_resolver() {
       ...
       * operations on the host resolver. The handler for each cancelled
       * operation will be invoked with the asio::error::operation_aborted error
       * code.
       */
      void cancel() {
      ...
      /// entries.
      /**
       * This function is used to resolve a query into a list of endpoint
       * entries.
       *
       * @param q A query object that determines what endpoints will be
       * returned.
       *
      ...
      results_type resolve(const query &q) {
        asio::error_code ec;
        results_type r =
            this->get_service().resolve(this->get_implementation(), q, ec);
       ...
       * or a numeric address string. If an empty string and the passive flag
       * has been specified, the resolved endpoints are suitable for local
       * service binding. If an empty string and passive is not specified, the
       * resolved endpoints will use the loopback address.
       *
       * @param service A string identifying the requested service. This may be
       * a descriptive name or a numeric string corresponding to a port number.
       * May be an empty string, in which case all resolved endpoints will have
       * a port number of 0.
       *
       * @returns A range object representing the list of endpoint entries. A
       * successful call to this function is guaranteed to return a non-empty
       ...
       * <tt>/etc/services</tt>. On Windows, service names may be found in the
       * file <tt>c:\\windows\\system32\\drivers\\etc\\services</tt>. Operating
       * systems may use additional locations when resolving service names.
       */
       ...
       * should be performed. The default flags are suitable for communication
       * with remote hosts.
       *
       * @returns A range object representing the list of endpoint entries. A
       * successful call to this function is guaranteed to return a non-empty
       ...
       * endpoint entries.
       *
       * @param protocol A protocol object, normally representing either the
       * IPv4 or IPv6 version of an internet protocol.
       *
       * @param host A string identifying a location. May be a descriptive name
       * or a numeric address string. If an empty string and the passive flag
       * has been specified, the resolved endpoints are suitable for local
      ...
      results_type resolve(const protocol_type &protocol,
                           ...
                           resolver_base::flags resolve_flags) {
        asio::error_code ec;
        basic_resolver_query<protocol_type> q(protocol,
      ...
      async_resolve(const query &q, ASIO_MOVE_ARG(ResolveHandler) handler) {
        // If you get an error on the following line it means that your handler
        // does not meet the documented type requirements for a ResolveHandler.
        ASIO_RESOLVE_HANDLER_CHECK(ResolveHandler, handler, results_type)
        type_check;
      ...
      /// Asynchronously perform forward resolution of a query to a list of
      /// entries.
      /**
       * This function is used to resolve host and service names into a list of
       * endpoint entries.
      ...
      results_type resolve(const endpoint_type &e) {
        asio::error_code ec;
        results_type i =
            this->get_service().resolve(this->get_implementation(), e, ec);
        asio::detail::throw_error(ec, "resolve");
        return i;
      }
