...
namespace asio {
  namespace ip {
    ...
    class network_v4 {
      public:
      /// Default constructor.
      network_v4() ASIO_NOEXCEPT : address_(), prefix_length_(0) {
      }
      ...
      address_v4 broadcast() const ASIO_NOEXCEPT {
        return address_v4(network().to_uint() |
                          (netmask().to_uint() ^ 0xFFFFFFFF));
      }
     ...
     * @relates address_v4
     */
    inline network_v4 make_network_v4(const address_v4 &addr,
    ...
    operator<<(std::basic_ostream<Elem, Traits> &os, const network_v4 &net);
