 ...
 */
...
#else  // defined(ASIO_NO_DEPRECATED) || defined(GENERATING_DOCUMENTATION)
    // For backward compatibility, determine the concrete completion handler
    // type by using the legacy handler_type trait.
  ...
  struct async_completion {
    ...
    typedef typename asio::async_result<typename decay<CompletionToken>::type,
    ...
    /// Constructor.
    /**
     * The constructor creates the concrete completion handler and makes the
     * link between the handler and the asynchronous result.
     */
    ...
    explicit async_completion(
        const typename decay<CompletionToken>::type &token)
        : completion_handler(token), result(completion_handler) {
    }
    ...
    typename conditional<
        is_same<CompletionToken, completion_handler_type>::value,
        completion_handler_type &,
        completion_handler_type>::type completion_handler;
#else  // defined(ASIO_HAS_MOVE) || defined(GENERATING_DOCUMENTATION)
...
#define ASIO_INITFN_RESULT_TYPE(ct, sig) \
  typename ::asio::detail::async_result_helper<ct, sig>::return_type
#define ASIO_HANDLER_TYPE(ct, sig) \
  typename ::asio::detail::async_result_helper<ct, sig>::completion_handler_type
#else
#define ASIO_INITFN_RESULT_TYPE(ct, sig)                          \
  typename ::asio::async_result<typename ::asio::decay<ct>::type, \
                                sig>::return_type
#define ASIO_HANDLER_TYPE(ct, sig)                                \
  typename ::asio::async_result<typename ::asio::decay<ct>::type, \
                                sig>::completion_handler_type
#endif
