...
namespace asio {
  ...
  template <typename Time,
            typename TimeTraits = asio::time_traits<Time> ASIO_SVC_TPARAM_DEF2(
                = deadline_timer_service<Time, TimeTraits>)>
  class basic_deadline_timer : ASIO_SVC_ACCESS basic_io_object<ASIO_SVC_T> {
    public:
     ...
     *
     * @param other The other basic_deadline_timer object from which the move
     * will occur.
     *
     * @note Following the move, the moved-from object is in the same state as
     * if constructed using the @c basic_deadline_timer(io_context&)
     * constructor.
     */
