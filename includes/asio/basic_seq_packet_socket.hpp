...
namespace asio {
  ...
  /**
   * The basic_seq_packet_socket class template provides asynchronous and
   * blocking sequenced packet socket functionality.
   *
   * @par Thread Safety
  ...
  class basic_seq_packet_socket : public basic_socket<Protocol ASIO_SVC_TARG> {
    public:
     ...
     * will use to dispatch handlers for any asynchronous operations performed
     * on the socket.
     */
     ...
     * opens it bound to the specified endpoint on the local machine. The
     * protocol used is the protocol associated with the given endpoint.
     *
     * @param io_context The io_context object that the sequenced packet socket
     * will use to dispatch handlers for any asynchronous operations performed
     * on the socket.
     ...
     * if constructed using the @c basic_seq_packet_socket(io_context&)
     * constructor.
     */
    ...
    operator=(basic_seq_packet_socket<Protocol1 ASIO_SVC_TARG1> &&other) {
      basic_socket<Protocol ASIO_SVC_TARG>::operator=(std::move(other));
      return *this;
    }
     ...
     * Consider using the @ref write function if you need to ensure that all
     ...
     * socket_base::message_end_of_record bit is set then the received data
     ...
     * peer.
     *
     * @note The receive operation may not receive all of the requested number
     ...
     * that they remain valid until the handler is called.
     *
     * @param out_flags Once the asynchronous operation completes, contains
     * flags associated with the received data. For example, if the
     * socket_base::message_end_of_record bit is set then the received data
     * marks the end of a record. The caller must guarantee that the referenced
    ...
    async_receive(const MutableBufferSequence &buffers,
      ...
      this->get_service().async_receive_with_flags(this->get_implementation(),
                                                   buffers,
                                                   0,
                                                   out_flags,
