    ...
    namespace detail {
      ...
      class redirect_error_handler {
        ...
        redirect_error_handler(redirect_error_t<CompletionToken> e)
            : ec_(e.ec_), handler_(ASIO_MOVE_CAST(CompletionToken)(e.token_)) {
        }
        ...
        typename enable_if<
            !is_same<typename decay<Arg>::type, asio::error_code>::value>::type
        operator()(ASIO_MOVE_ARG(Arg) arg, ASIO_MOVE_ARG(Args)... args) {
          handler_(ASIO_MOVE_CAST(Arg)(arg), ASIO_MOVE_CAST(Args)(args)...);
        }
...
#define ASIO_PRIVATE_REDIRECT_ERROR_DEF(n)                                    \
  ...
  operator()(ASIO_MOVE_ARG(Arg) arg, ASIO_VARIADIC_MOVE_PARAMS(n)) {          \
    handler_(ASIO_MOVE_CAST(Arg)(arg), ASIO_VARIADIC_MOVE_ARGS(n));           \
  }                                                                           \
                                                                              \
...
#undef ASIO_PRIVATE_REDIRECT_ERROR_DEF
  ...
  struct async_result<experimental::redirect_error_t<CompletionToken>,
                      Signature>
      : async_result<CompletionToken,
                     typename experimental::detail::redirect_error_signature<
                         Signature>::type> {
    typedef experimental::detail::redirect_error_handler<typename async_result<
        CompletionToken,
        typename experimental::detail::redirect_error_signature<
            Signature>::type>::completion_handler_type>
  ...
  template <typename CompletionToken, typename Signature>
  struct handler_type<experimental::redirect_error_t<CompletionToken>,
                      Signature> {
    typedef experimental::detail::redirect_error_handler<typename async_result<
        CompletionToken,
        typename experimental::detail::redirect_error_signature<
  ...
  struct associated_executor<
      experimental::detail::redirect_error_handler<Handler>,
      Executor> {
    typedef typename associated_executor<Handler, Executor>::type type;

    static type
    get(const experimental::detail::redirect_error_handler<Handler> &h,
