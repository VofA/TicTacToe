  ...
  namespace experimental {
    namespace detail {
      ...
      class awaiter {
        ...
        ~awaiter() {
          if (has_executor_)
            static_cast<Executor *>(static_cast<void *>(executor_))
                ->~Executor();
        }
        ...
        executor_type get_executor() const noexcept {
          return *static_cast<const Executor *>(
              static_cast<const void *>(executor_));
        }
      ...
      // Base promise for coroutines further down the thread-of-execution
      // "stack".
      template <typename Executor>
      class awaitee_base {
        public:
        ...
        class awaitable_executor {
          public:
          explicit awaitable_executor(awaitee_base *a) : this_(a) {
          }
          ...
          void
          await_suspend(coroutine_handle<detail::awaitee<U, Ex>> h) noexcept {
        ...
        class awaitable_token {
          public:
          explicit awaitable_token(awaitee_base *a) : this_(a) {
          }
        ...
        await_transform(awaitable<T, Executor> &t) const {
          return std::move(t);
        }
        ...
        await_transform(std::experimental::suspend_always) const {
          return std::experimental::suspend_always();
        }
      ...
      // Promise object for coroutines further down the thread-of-execution
      // "stack".
      template <typename T, typename Executor>
      class awaitee : public awaitee_base<Executor> {
        public:
        awaitee() {
        }
      ...
      // "stack".
      template <typename Executor>
      class awaitee<void, Executor> : public awaitee_base<Executor> {
        public:
      ...
      class awaiter_task {
        ...
        awaiter_task(awaiter<Executor> *a) : awaiter_(a->add_ref()) {
        }
        ...
        ~awaiter_task() {
          if (awaiter_) {
            // Coroutine "stack unwinding" must be performed through the
            // executor.
            executor_type ex(awaiter_->get_executor());
            (post)(ex, [a = std::move(awaiter_)]() mutable {
      ...
      class co_spawn_handler : public awaiter_task<Executor> {
        ...
        void operator()() {
          ...
          coroutine_handle<awaiter<Executor>>::from_promise(*ptr.get())
              .resume();
        }
      };
      ...
      class await_handler_base : public awaiter_task<Executor> {
        ...
        await_handler_base(await_token<Executor> token)
            : awaiter_task<Executor>(token.awaiter_), awaitee_(nullptr) {
        }
      ...
      class await_handler
        ...
        public:
        using await_handler_base<Executor,
                                 std::tuple<Ts...>>::await_handler_base;
      ...
      co_spawn_entry_point(awaitable<T, Executor> *,
                           executor_work_guard<Executor> work_guard,
                           F f,
                           Handler handler) {
        bool done = false;
        ...
        try {
          ...
          (dispatch)(
              ...
              [handler = std::move(handler),
               e = std::current_exception()]() mutable { handler(e, T()); });
        }
      }

      template <typename Executor, typename F, typename Handler>
      awaiter<Executor> *
      co_spawn_entry_point(awaitable<void, Executor> *,
                           executor_work_guard<Executor> work_guard,
                           F f,
                           Handler handler) {
      ...
      auto co_spawn(const Executor &ex, F &&f, CompletionToken &&token) {
        ...
        typedef
            typename awaitable_signature<awaitable_type>::type signature_type;
        ...
        auto *a =
            (co_spawn_entry_point)(static_cast<awaitable_type *>(nullptr),
                                   std::move(work_guard),
                                   std::forward<F>(f),
...
#pragma warning(pop)
  ...
  class async_result<experimental::await_token<Executor>, R(Args...)> {
    public:
    typedef experimental::detail::await_handler<Executor,
                                                typename decay<Args>::type...>
        completion_handler_type;

    typedef
        typename experimental::detail::await_handler<Executor,
                                                     Args...>::awaitable_type
            return_type;

    async_result(completion_handler_type &h)
        : awaitable_(
              experimental::detail::make_dummy_awaitable<return_type>()) {
...
namespace std {
  namespace experimental {
    ...
    template <typename Executor, typename... Args>
    struct coroutine_traits<asio::experimental::detail::awaiter<Executor> *,
                            Args...> {
      typedef asio::experimental::detail::awaiter<Executor> promise_type;
    };

    template <typename T, typename Executor, typename... Args>
    struct coroutine_traits<asio::experimental::awaitable<T, Executor>,
                            Args...> {
