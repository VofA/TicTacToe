...
namespace asio {
  namespace experimental {
    ...
    namespace this_coro {
      ...
      /// Awaitable type that returns a completion token for the current
      /// coroutine.
      struct token_t {};

      /// Awaitable object that returns a completion token for the current
      /// coroutine.
     ...
     * } @endcode
     *
     * The initiating function (async_read_some in the above example) suspends
     * the current coroutine. The coroutine is resumed when the asynchronous
     * operation completes, and the result of the operation is returned.
     */
    ...
    class await_token {
      ...
      template <typename>
      friend class detail::awaitee_base;
      template <typename, typename>
      friend class detail::await_handler_base;

      // Private constructor used by awaitee_base.
      explicit await_token(detail::awaiter<Executor> *a) : awaiter_(a) {
      }
    ...
    class awaitable {
      ...
      ~awaitable() {
        if (awaitee_) {
          detail::coroutine_handle<detail::awaitee<T, Executor>>::from_promise(
              *awaitee_)
              .destroy();
        }
      }
      ...
      // Support for co_await keyword.
      void
      await_suspend(detail::coroutine_handle<detail::awaiter<Executor>> h) {
      ...
      template <class U>
      void
      await_suspend(detail::coroutine_handle<detail::awaitee<U, Executor>> h) {
        awaitee_->attach_caller(h);
      }
    ...
    inline auto co_spawn(const Executor &ex, F &&f, CompletionToken &&token) {
      return detail::co_spawn(
          ex, std::forward<F>(f), std::forward<CompletionToken>(token));
    }
    ...
    template <typename ExecutionContext,
              ...
              typename = typename enable_if<
                  is_convertible<ExecutionContext &,
                                 execution_context &>::value>::type>
    inline auto
    co_spawn(ExecutionContext &ctx, F &&f, CompletionToken &&token) {
      return detail::co_spawn(ctx.get_executor(),
                              std::forward<F>(f),
    ...
    inline auto co_spawn(const await_token<Executor> &parent,
      ...
      return detail::co_spawn(parent.get_executor(),
                              std::forward<F>(f),
                              std::forward<CompletionToken>(token));
    }
