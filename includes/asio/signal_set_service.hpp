...
namespace asio {
...
#endif
  {
    ...
    ASIO_SYNC_OP_VOID
    add(implementation_type &impl, int signal_number, asio::error_code &ec) {
    ...
    ASIO_SYNC_OP_VOID
    remove(implementation_type &impl, int signal_number, asio::error_code &ec) {
      service_impl_.remove(impl, signal_number, ec);
      ASIO_SYNC_OP_VOID_RETURN(ec);
    }

    /// Remove all signals from a signal_set.
    ASIO_SYNC_OP_VOID clear(implementation_type &impl, asio::error_code &ec) {
    ...
    ASIO_INITFN_RESULT_TYPE(SignalHandler, void(asio::error_code, int))
    async_wait(implementation_type &impl,
               ASIO_MOVE_ARG(SignalHandler) handler) {
      async_completion<SignalHandler, void(asio::error_code, int)> init(
          handler);
