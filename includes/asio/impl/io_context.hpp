...
namespace asio {
  ...
  inline io_context::executor_type io_context::get_executor() ASIO_NOEXCEPT {
  ...
  std::size_t
  io_context::run_for(const chrono::duration<Rep, Period> &rel_time) {
    return this->run_until(chrono::steady_clock::now() + rel_time);
  }

  template <typename Clock, typename Duration>
  std::size_t
  io_context::run_until(const chrono::time_point<Clock, Duration> &abs_time) {
    std::size_t n = 0;
  ...
  template <typename Rep, typename Period>
  std::size_t
  io_context::run_one_for(const chrono::duration<Rep, Period> &rel_time) {
  ...
  io_context::dispatch(ASIO_MOVE_ARG(LegacyCompletionHandler) handler) {
    ...
    ASIO_LEGACY_COMPLETION_HANDLER_CHECK(LegacyCompletionHandler, handler)
    type_check;
    ...
    if (impl_.can_dispatch()) {
      ...
      asio_handler_invoke_helpers::invoke(init.completion_handler,
                                          init.completion_handler);
    } else {
      // Allocate and construct an operation to wrap the handler.
      typedef detail::completion_handler<
          typename handler_type<LegacyCompletionHandler, void()>::type>
          op;
      typename op::ptr p = {detail::addressof(init.completion_handler),
                            op::ptr::allocate(init.completion_handler),
                            0};
      p.p                = new (p.v) op(init.completion_handler);

      ASIO_HANDLER_CREATION((*this, *p.p, "io_context", this, 0, "dispatch"));
  ...
  io_context::post(ASIO_MOVE_ARG(LegacyCompletionHandler) handler) {
    ...
    ASIO_HANDLER_CREATION((*this, *p.p, "io_context", this, 0, "post"));
  ...
  inline io_context &io_context::executor_type::context() const ASIO_NOEXCEPT {
    return io_context_;
  }

  inline void io_context::executor_type::on_work_started() const ASIO_NOEXCEPT {
  ...
  void io_context::executor_type::dispatch(ASIO_MOVE_ARG(Function) f,
    ...
    ASIO_HANDLER_CREATION(
        (this->context(), *p.p, "io_context", &this->context(), 0, "post"));
  ...
  template <typename Function, typename Allocator>
  void io_context::executor_type::post(ASIO_MOVE_ARG(Function) f,
                                       const Allocator &a) const {
    typedef typename decay<Function>::type function_type;
  ...
  template <typename Function, typename Allocator>
  void io_context::executor_type::defer(ASIO_MOVE_ARG(Function) f,
                                        const Allocator &a) const {
    typedef typename decay<Function>::type function_type;
    ...
    ASIO_HANDLER_CREATION(
        (this->context(), *p.p, "io_context", &this->context(), 0, "defer"));
