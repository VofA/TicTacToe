...
#define ASIO_PRIVATE_PROMISE_INVOKE_DEF(n)                         \
  ...
  inline void promise_invoke_and_set(                              \
      std::promise<T> &p, F &f, ASIO_VARIADIC_MOVE_PARAMS(n)) {    \
    p.set_value(f(ASIO_VARIADIC_MOVE_ARGS(n)));                    \
  }                                                                \
                                                                   \
  template <typename F, ASIO_VARIADIC_TPARAMS(n)>                  \
  inline void promise_invoke_and_set(                              \
      std::promise<void> &p, F &f, ASIO_VARIADIC_MOVE_PARAMS(n)) { \
...
#undef ASIO_PRIVATE_PROMISE_INVOKE_DEF
    ...
    class promise_invoker {
      public:
      promise_invoker(const shared_ptr<std::promise<T>> &p, ASIO_MOVE_ARG(F) f)
    ...
    // An executor that adapts the system_executor to capture any exeption
    // thrown by a submitted function object and save it into a promise.
    template <typename T>
    class promise_executor {
      ...
      void post(ASIO_MOVE_ARG(F) f, const A &a) const {
        system_executor().post(promise_invoker<T, F>(p_, ASIO_MOVE_CAST(F)(f)),
                               a);
      }

      template <typename F, typename A>
      void defer(ASIO_MOVE_ARG(F) f, const A &a) const {
        system_executor().defer(promise_invoker<T, F>(p_, ASIO_MOVE_CAST(F)(f)),
                                a);
      }
    ...
    class promise_handler_ec_1 : public promise_creator<T> {
      public:
      template <typename Arg>
      void operator()(const asio::error_code &ec, ASIO_MOVE_ARG(Arg) arg) {
        if (ec) {
          this->p_->set_exception(
              std::make_exception_ptr(asio::system_error(ec)));
        } else
    ...
    class promise_handler_ex_1 : public promise_creator<T> {
      public:
      template <typename Arg>
      void operator()(const std::exception_ptr &ex, ASIO_MOVE_ARG(Arg) arg) {
        if (ex)
          this->p_->set_exception(ex);
        else
          this->p_->set_value(ASIO_MOVE_CAST(Arg)(arg));
    ...
    template <typename T>
    class promise_handler_n : public promise_creator<T> {
      public:
#if defined(ASIO_HAS_VARIADIC_TEMPLATES)
      ...
      void operator()(ASIO_MOVE_ARG(Args)... args) {
        this->p_->set_value(
            std::forward_as_tuple(ASIO_MOVE_CAST(Args)(args)...));
      }
...
#define ASIO_PRIVATE_CALL_OP_DEF(n)                                         \
  ...
  void operator()(ASIO_VARIADIC_MOVE_PARAMS(n)) {                           \
    this->p_->set_value(std::forward_as_tuple(ASIO_VARIADIC_MOVE_ARGS(n))); \
  }                                                                         \
  /**/
...
#undef ASIO_PRIVATE_CALL_OP_DEF
    ...
    template <typename T>
    class promise_handler_ec_n : public promise_creator<T> {
      public:
#if defined(ASIO_HAS_VARIADIC_TEMPLATES)

      template <typename... Args>
      void operator()(const asio::error_code &ec, ASIO_MOVE_ARG(Args)... args) {
        if (ec) {
          this->p_->set_exception(
              std::make_exception_ptr(asio::system_error(ec)));
        } else {
...
#define ASIO_PRIVATE_CALL_OP_DEF(n)                                           \
  template <ASIO_VARIADIC_TPARAMS(n)>                                         \
  void operator()(const asio::error_code &ec, ASIO_VARIADIC_MOVE_PARAMS(n)) { \
    if (ec) {                                                                 \
      this->p_->set_exception(                                                \
          std::make_exception_ptr(asio::system_error(ec)));                   \
    } else {                                                                  \
...
#undef ASIO_PRIVATE_CALL_OP_DEF
    ...
    template <typename T>
    class promise_handler_ex_n : public promise_creator<T> {
      public:
#if defined(ASIO_HAS_VARIADIC_TEMPLATES)
...
#define ASIO_PRIVATE_PROMISE_SELECTOR_DEF(n)                                   \
  ...
  class promise_handler_selector<void(Arg, ASIO_VARIADIC_TARGS(n))>            \
      : public promise_handler_n<std::tuple<Arg, ASIO_VARIADIC_TARGS(n)>> {};  \
                                                                               \
  template <typename Arg, ASIO_VARIADIC_TPARAMS(n)>                            \
  class promise_handler_selector<void(                                         \
      asio::error_code, Arg, ASIO_VARIADIC_TARGS(n))>                          \
      : public promise_handler_ec_n<std::tuple<Arg, ASIO_VARIADIC_TARGS(n)>> { \
  };                                                                           \
                                                                               \
  template <typename Arg, ASIO_VARIADIC_TPARAMS(n)>                            \
  class promise_handler_selector<void(                                         \
      std::exception_ptr, Arg, ASIO_VARIADIC_TARGS(n))>                        \
      : public promise_handler_ex_n<std::tuple<Arg, ASIO_VARIADIC_TARGS(n)>> { \
  };                                                                           \
  /**/
...
#undef ASIO_PRIVATE_PROMISE_SELECTOR_DEF
    ...
    inline void asio_handler_invoke(Function &f,
      ...
      typename promise_handler<Signature, Allocator>::executor_type ex(
          h->get_executor());
    ...
    class packaged_token {
      ...
      packaged_token(Function f, const Allocator &a)
          : function_(ASIO_MOVE_CAST(Function)(f)), allocator_(a) {
      }
    ...
    class packaged_handler : public promise_creator<Result> {
      ...
      void operator()(ASIO_MOVE_ARG(Args)... args) {
        (promise_invoke_and_set)(
            *this->p_, function_, ASIO_MOVE_CAST(Args)(args)...);
      }
...
#define ASIO_PRIVATE_CALL_OP_DEF(n)                        \
  ...
  void operator()(ASIO_VARIADIC_MOVE_PARAMS(n)) {          \
    (promise_invoke_and_set)(                              \
        *this->p_, function_, ASIO_VARIADIC_MOVE_ARGS(n)); \
  }                                                        \
  /**/
      ASIO_VARIADIC_GENERATE(ASIO_PRIVATE_CALL_OP_DEF)
#undef ASIO_PRIVATE_CALL_OP_DEF
    ...
    asio_handler_invoke(Function &f,
      ...
      typename packaged_handler<Function1, Allocator, Result>::executor_type ex(
          h->get_executor());
      ex.dispatch(ASIO_MOVE_CAST(Function)(f), std::allocator<void>());
    }
    ...
    template <typename Function,
              ...
              typename Result>
    inline void
    asio_handler_invoke(const Function &f,
                        packaged_handler<Function1, Allocator, Result> *h) {
      typename packaged_handler<Function1, Allocator, Result>::executor_type ex(
          h->get_executor());
      ex.dispatch(f, std::allocator<void>());
    ...
    class packaged_async_result {
      public:
      typedef packaged_handler<Function, Allocator, Result>
  ...
  class async_result<use_future_t<Allocator>, Result(Args...)>
      : public detail::promise_async_result<void(typename decay<Args>::type...),
                                            Allocator> {
    public:
    explicit async_result(typename detail::promise_async_result<
                          void(typename decay<Args>::type...),
                          Allocator>::completion_handler_type &h)
        : detail::promise_async_result<void(typename decay<Args>::type...),
                                       Allocator>(h) {
    }
  };
  ...
  class async_result<detail::packaged_token<Function, Allocator>,
    ...
    public:
    explicit async_result(typename detail::packaged_async_result<
                          Function,
                          Allocator,
                          typename result_of<Function(Args...)>::type>::
                              completion_handler_type &h)
        : detail::packaged_async_result<
              Function,
              Allocator,
  ...
  class async_result<detail::packaged_token<Function, Allocator>, Result()>
    ...
    explicit async_result(
        ...
        : detail::packaged_async_result<Function,
                                        Allocator,
                                        typename result_of<Function()>::type>(
              h) {
    }
...
#define ASIO_PRIVATE_ASYNC_RESULT_DEF(n)                                       \
  template <typename Allocator, typename Result, ASIO_VARIADIC_TPARAMS(n)>     \
  class async_result<use_future_t<Allocator>, Result(ASIO_VARIADIC_TARGS(n))>  \
      : public detail::promise_async_result<void(ASIO_VARIADIC_DECAY(n)),      \
                                            Allocator> {                       \
    public:                                                                    \
    explicit async_result(typename detail::promise_async_result<               \
                          void(ASIO_VARIADIC_DECAY(n)),                        \
                          Allocator>::completion_handler_type &h)              \
        : detail::promise_async_result<void(ASIO_VARIADIC_DECAY(n)),           \
                                       Allocator>(h) {                         \
    }                                                                          \
  };                                                                           \
                                                                               \
  ...
  class async_result<detail::packaged_token<Function, Allocator>,              \
    ...
    explicit async_result(typename detail::packaged_async_result<              \
                          Function,                                            \
                          Allocator,                                           \
                          typename result_of<Function(ASIO_VARIADIC_TARGS(     \
...
#undef ASIO_PRIVATE_ASYNC_RESULT_DEF
  ...
  class async_result<detail::promise_handler<Signature, Allocator>>
    ...
    public:
    typedef
        typename detail::promise_async_result<Signature, Allocator>::return_type
            type;
  ...
  class async_result<detail::packaged_handler<Function, Allocator, Result>>
    ...
    typedef typename detail::
        packaged_async_result<Function, Allocator, Result>::return_type type;

    explicit async_result(
        typename detail::packaged_async_result<Function, Allocator, Result>::
            completion_handler_type &h)
