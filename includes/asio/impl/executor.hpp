...
namespace asio {
  ...
  class executor::function {
    ...
    function(function &&other) : op_(other.op_) {
      other.op_ = 0;
    }
  ...
  class executor::impl : public executor::impl_base {
    ...
    struct raw_mem {
      ...
      explicit raw_mem(const Allocator &a)
          : allocator_(a), ptr_(allocator_.allocate(1)) {
      }
  ...
  void executor::dispatch(ASIO_MOVE_ARG(Function) f, const Allocator &a) const {
  ...
  template <typename Function, typename Allocator>
  void executor::post(ASIO_MOVE_ARG(Function) f, const Allocator &a) const {
    get_impl()->post(function(ASIO_MOVE_CAST(Function)(f), a));
  }

  template <typename Function, typename Allocator>
  void executor::defer(ASIO_MOVE_ARG(Function) f, const Allocator &a) const {
  ...
  Executor *executor::target() ASIO_NOEXCEPT {
    return impl_ && impl_->target_type() == type_id<Executor>()
               ? static_cast<Executor *>(impl_->target())
               : 0;
  }
