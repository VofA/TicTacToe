...
namespace asio {
  ...
  /**
   * The basic_socket class template provides functionality that is common to
   * both stream-oriented and datagram-oriented sockets.
   *
   * @par Thread Safety
  ...
  class basic_socket : ASIO_SVC_ACCESS basic_io_object<ASIO_SVC_T>,
                       public socket_base {
    public:
     ...
     */
    basic_socket(asio::io_context &io_context, const protocol_type &protocol)
        : basic_io_object<ASIO_SVC_T>(io_context) {
      asio::error_code ec;
     ...
     * specified endpoint on the local machine. The protocol used is the
     ...
     * @throws asio::system_error Thrown on failure.
     */
    basic_socket(asio::io_context &io_context, const endpoint_type &endpoint)
        : basic_io_object<ASIO_SVC_T>(io_context) {
      asio::error_code ec;
    ...
    /**
     * This constructor creates a socket object to hold an existing native
     * socket.
     *
     * @param io_context The io_context object that the socket will use to
     * dispatch handlers for any asynchronous operations performed on the
    ...
    basic_socket(asio::io_context &io_context,
      ...
      this->get_service().assign(
          this->get_implementation(), protocol, native_socket, ec);
     ...
     * if constructed using the @c basic_socket(io_context&) constructor.
     */
    ...
    basic_socket(
        ...
        typename enable_if<is_convertible<Protocol1, Protocol>::value>::type * =
            0)
        : basic_io_object<ASIO_SVC_T>(other.get_service(),
    ...
    typename enable_if<is_convertible<Protocol1, Protocol>::value,
                       basic_socket>::type &
    operator=(basic_socket<Protocol1 ASIO_SVC_TARG1> &&other) {
    ...
    /**
     * This function opens the socket so that it will use the specified
     * protocol.
     *
     * @param protocol An object specifying protocol parameters to be used.
     *
     ...
     * Ownership of the native socket is then transferred to the caller.
     *
     * @throws asio::system_error Thrown on failure.
     *
    ...
    __declspec(
        deprecated("This function always fails with "
     ...
     *
     * @li It will only cancel asynchronous operations that were initiated in
     * the current thread.
     *
     * @li It can appear to complete without error, but the request to cancel
     * the unfinished operations may be silently ignored by the operating
     * system. Whether it works or not seems to depend on the drivers that are
     * installed.
     *
    ...
    std::size_t available() const {
      asio::error_code ec;
      std::size_t s =
          this->get_service().available(this->get_implementation(), ec);
    ...
    /**
     * This function is used to connect a socket to the specified remote
    ...
    void connect(const endpoint_type &peer_endpoint) {
      ...
      if (!is_open()) {
        this->get_service().open(
            this->get_implementation(), peer_endpoint.protocol(), ec);
        asio::detail::throw_error(ec, "connect");
      }
      this->get_service().connect(
    ...
    async_connect(const endpoint_type &peer_endpoint,
      ...
      if (!is_open()) {
        ...
        this->get_service().open(this->get_implementation(), protocol, ec);
        if (ec) {
          async_completion<ConnectHandler, void(asio::error_code)> init(
              handler);
...
#else  // defined(ASIO_ENABLE_OLD_SERVICES)
      async_completion<ConnectHandler, void(asio::error_code)> init(handler);
     ...
     * asynchronous operations. Asynchronous operations will never fail with the
    ...
    /**
     * @param mode If @c true, the socket's synchronous operations will fail
     * with asio::error::would_block if they are unable to perform the requested
     * operation immediately. If @c false, synchronous operations will block
     * until complete.
     *
     * @throws asio::system_error Thrown on failure.
    ...
    endpoint_type local_endpoint() const {
      asio::error_code ec;
      endpoint_type ep =
          this->get_service().local_endpoint(this->get_implementation(), ec);
    ...
    endpoint_type remote_endpoint() const {
      asio::error_code ec;
      endpoint_type ep =
          this->get_service().remote_endpoint(this->get_implementation(), ec);
    ...
    endpoint_type remote_endpoint(asio::error_code &ec) const {
      return this->get_service().remote_endpoint(this->get_implementation(),
                                                 ec);
    }
    ...
    /**
     * This function is used to perform an asynchronous wait for a socket to
    ...
    async_wait(wait_type w, ASIO_MOVE_ARG(WaitHandler) handler) {
      ...
      return this->get_service().async_wait(
          this->get_implementation(), w, ASIO_MOVE_CAST(WaitHandler)(handler));
#else  // defined(ASIO_ENABLE_OLD_SERVICES)
     ...
     * asynchronous operations associated with the socket as if by calling @c
     * cancel.
     */
    ~basic_socket() {
    }
