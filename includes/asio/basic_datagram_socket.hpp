...
namespace asio {
  ...
  template <typename Protocol ASIO_SVC_TPARAM_DEF1(
      = datagram_socket_service<Protocol>)>
  class basic_datagram_socket : public basic_socket<Protocol ASIO_SVC_TARG> {
    public:
    /// The native representation of a socket.
#if defined(GENERATING_DOCUMENTATION)
    typedef implementation_defined native_handle_type;
#else
    typedef typename basic_socket<Protocol ASIO_SVC_TARG>::native_handle_type
        native_handle_type;
#endif
     ...
     * function must be called before data can be sent or received on the
     * socket.
     *
    ...
    /// Construct a basic_datagram_socket, opening it and binding it to the
     ...
     * if constructed using the @c basic_datagram_socket(io_context&)
     * constructor.
     */
    ...
    typename enable_if<is_convertible<Protocol1, Protocol>::value,
                       basic_datagram_socket>::type &
    operator=(basic_datagram_socket<Protocol1 ASIO_SVC_TARG1> &&other) {
      basic_socket<Protocol ASIO_SVC_TARG>::operator=(std::move(other));
      return *this;
    }
    ...
    std::size_t send(const ConstBufferSequence &buffers) {
      asio::error_code ec;
      std::size_t s =
          this->get_service().send(this->get_implementation(), buffers, 0, ec);
     ...
     *
     * @param handler The handler to be called when the send operation
    ...
    async_send(const ConstBufferSequence &buffers,
      ...
      async_completion<WriteHandler, void(asio::error_code, std::size_t)> init(
          handler);

      this->get_service().async_send(
          this->get_implementation(), buffers, 0, init.completion_handler);
    ...
    async_send(const ConstBufferSequence &buffers,
      ...
      this->get_service().async_send(
          this->get_implementation(), buffers, flags, init.completion_handler);
    ...
    /**
     * This function is used to send a datagram to the specified remote
     * endpoint. The function call will block until the data has been sent
     * successfully or an error occurs.
     *
     * @param buffers One or more data buffers to be sent to the remote
     * endpoint.
     *
    ...
    std::size_t send_to(const ConstBufferSequence &buffers,
                        ...
                        socket_base::message_flags flags,
                        asio::error_code &ec) {
      return this->get_service().send_to(
          this->get_implementation(), buffers, destination, flags, ec);
    }
     ...
     *
     * @param buffers One or more data buffers to be sent to the remote
     * endpoint. Although the buffers object may be copied as necessary,
    ...
    /**
     * This function is used to receive data on the datagram socket. The
     ...
     * using asio::io_context::post().
     *
     * @note The async_receive operation can only be used with a connected
     * socket. Use the async_receive_from function to receive data on an
...
#else  // defined(ASIO_ENABLE_OLD_SERVICES)
      async_completion<ReadHandler, void(asio::error_code, std::size_t)> init(
          handler);

      this->get_service().async_receive(
          this->get_implementation(), buffers, 0, init.completion_handler);
    ...
    std::size_t receive_from(const MutableBufferSequence &buffers,
                             ...
                             socket_base::message_flags flags,
                             asio::error_code &ec) {
      return this->get_service().receive_from(
          this->get_implementation(), buffers, sender_endpoint, flags, ec);
    }
     ...
     *
     * @param sender_endpoint An endpoint object that receives the endpoint of
     * the remote sender of the datagram. Ownership of the sender_endpoint
    ...
    async_receive_from(const MutableBufferSequence &buffers,
      ...
      this->get_service().async_receive_from(this->get_implementation(),
                                             buffers,
                                             sender_endpoint,
                                             0,
                                             init.completion_handler);
