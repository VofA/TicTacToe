...
namespace asio {
  ...
  /// can be used with with @c connect and @c async_connect.
  template <typename T>
   ...
   * The function object should return true if the next endpoint should be
   * tried, and false if it should be skipped.
   *
   * @returns The successfully connected endpoint.
   *
   * @throws asio::system_error Thrown on failure. If the sequence is
  ...
  async_connect(basic_socket<Protocol ASIO_SVC_TARG> &s,
                ...
                typename enable_if<
                    is_endpoint_sequence<EndpointSequence>::value>::type * = 0);
