...
namespace asio {
  ...
  write(SyncWriteStream &s,
        ...
        typename enable_if<
            is_const_buffer_sequence<ConstBufferSequence>::value>::type * = 0);
   ...
   * on the next call to the stream's write_some function.
   *
   * @returns The number of bytes transferred.
   *
   * @throws asio::system_error Thrown on failure.
   ...
   * stream. The call will block until one of the following conditions is true:
   *
   * @li All of the data in the supplied dynamic buffer sequence has been
   * written.
   *
   * @li An error occurred.
   *
   ...
   *
   * @brief Start an asynchronous operation to write a certain amount of data to
   * a stream.
   */
  /*@{*/
   ...
   *
   * This operation is implemented in terms of zero or more calls to the
   * stream's async_write_some function, and is known as a <em>composed
   ...
   *
   * @param s The stream to which the data is to be written. The type must
   * support the AsyncWriteStream concept.
   *
   ...
   * A return value of 0 indicates that the write operation is complete. A
   * non-zero return value indicates the maximum number of bytes to be written
   * on the next call to the stream's async_write_some function.
   *
   * @param handler The handler to be called when the write operation completes.
   * Copies will be made of the handler as required. The function signature of
   * the handler must be:
