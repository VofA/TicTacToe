...
namespace asio {
   ...
   * invoked, in particular that a handler can only be invoked from a thread
   * that is currently calling @c run() on the corresponding io_context object.
   ...
   * If necessary, the default implementation makes a copy of the function
   * object so that the non-const operator() can be used.
   *
   * @par Example
   * @code
   * class my_handler;
