...
#define ASIO_SVC_T \
  detail::deadline_timer_service<detail::chrono_time_traits<Clock, WaitTraits>>
...
#define ASIO_BASIC_WAITABLE_TIMER_FWD_DECL
  ...
  template <typename Clock,
            typename WaitTraits = asio::wait_traits<Clock> ASIO_SVC_TPARAM_DEF2(
   ...
   * expired". If the wait() or async_wait() function is called on an expired
   * timer, the wait operation will complete immediately.
   *
   ...
   * action associated with the timer is performed only once, use something like
   * this: used:
   *
   * @code
   * void on_some_event()
  ...
  class basic_waitable_timer : ASIO_SVC_ACCESS basic_io_object<ASIO_SVC_T> {
    public:
     ...
     *
     * @param io_context The io_context object that the timer will use to
     ...
     *
     * @param other The other basic_waitable_timer object from which the move
     * will occur.
     *
     * @note Following the move, the moved-from object is in the same state as
     * if constructed using the @c basic_waitable_timer(io_context&)
     * constructor.
     */
     ...
     * Cancelling the timer does not change the expiry time.
     *
     * @return The number of asynchronous operations that were cancelled. That
     * is, either 0 or 1.
     *
     * @throws asio::system_error Thrown on failure.
     *
    ...
    std::size_t cancel_one() {
      asio::error_code ec;
      std::size_t s =
          this->get_service().cancel_one(this->get_implementation(), ec);
     ...
     *
     * @li The timer was cancelled, in which case the handler is passed the
    ...
    async_wait(ASIO_MOVE_ARG(WaitHandler) handler) {
      ...
      return this->get_service().async_wait(
          this->get_implementation(), ASIO_MOVE_CAST(WaitHandler)(handler));
#else  // defined(ASIO_ENABLE_OLD_SERVICES)
    ...
    basic_waitable_timer &operator=(const basic_waitable_timer &) ASIO_DELETED;
  };
