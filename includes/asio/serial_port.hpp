...
namespace asio {
   ...
   */
  class serial_port : ASIO_SVC_ACCESS basic_io_object<ASIO_SVC_T>,
     ...
     * port.
     */
    explicit serial_port(asio::io_context &io_context, const char *device)
        : basic_io_object<ASIO_SVC_T>(io_context) {
      asio::error_code ec;
    ...
    serial_port(asio::io_context &io_context,
      ...
      this->get_service().assign(
          this->get_implementation(), native_serial_port, ec);
     ...
     * if constructed using the @c serial_port(io_context&) constructor.
     */
    ...
    /*
     * This function opens the serial port to hold an existing native serial
     * port.
     *
     * @param native_serial_port A native serial port.
     *
