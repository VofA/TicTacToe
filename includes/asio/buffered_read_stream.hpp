...
namespace asio {
  ...
  class buffered_read_stream : private noncopyable {
    public:
    ...
    std::size_t copy(const MutableBufferSequence &buffers) {
      std::size_t bytes_copied =
          asio::buffer_copy(buffers, storage_.data(), storage_.size());
      storage_.consume(bytes_copied);
      return bytes_copied;
    }

    /// Copy data from the internal buffer to the specified target buffer,
    /// without removing the data from the internal buffer. Returns the number
    /// of bytes copied.
    template <typename MutableBufferSequence>
