...
#if defined(ASIO_HAS_STD_FUTURE) || defined(GENERATING_DOCUMENTATION)
...
namespace asio {
   ...
   * should return a std::future object. A use_future_t object may be passed as
   * a handler to an asynchronous operation, typically using the special value
   * @c asio::use_future. For example:
   *
  ...
  class use_future_t {
    ...
    operator[](const OtherAllocator &allocator) const {
     ...
     * task. When this adapter is passed as a completion token to an
    ...
    private:
    // Helper type to ensure that use_future can be constexpr
