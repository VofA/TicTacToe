...
namespace asio {
   ...
   * } @endcode
   *
   * When @c reenter is executed at runtime, control jumps to the location of
   * the last @c yield or @c fork.
   *
   ...
   * function object object with a coroutine object as base class or data
   * member.
   *
   ...
   * @li Control is transferred to the end of the coroutine body.
   *
   * Once terminated, calls to is_complete() return true and the coroutine
   * cannot be reentered.
   *
   ...
   * splitting it into two (or more) copies. One use of @c fork is in a server,
   * where a new coroutine is created to handle each client connection:
   *
   * @code reenter (this)
   * {
   *   do
   ...
   * application's responsibility to create a clone of the coroutine and call
