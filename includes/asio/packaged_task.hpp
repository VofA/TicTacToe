...
#define ASIO_PRIVATE_ASYNC_RESULT_DEF(n)                                   \
  template <typename Result, ASIO_VARIADIC_TPARAMS(n), typename Signature> \
  class async_result<std::packaged_task<Result(ASIO_VARIADIC_TARGS(n))>,   \
                     Signature> {                                          \
    public:                                                                \
    typedef std::packaged_task<Result(ASIO_VARIADIC_TARGS(n))>             \
