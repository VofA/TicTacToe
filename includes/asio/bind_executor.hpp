...
namespace asio {
  namespace detail {
    ...
    class executor_binder_base<T, Executor, true> : protected Executor {
      protected:
    ...
    class executor_binder_base<T, Executor, false> {
      ...
      template <typename E, typename U>
      executor_binder_base(ASIO_MOVE_ARG(E) e, ASIO_MOVE_ARG(U) u)
          : executor_(ASIO_MOVE_CAST(E)(e)), target_(ASIO_MOVE_CAST(U)(u)) {
      }
  ...
  /// A call wrapper type to bind an executor of type @c Executor to an object
  /// of type @c T.
  template <typename T, typename Executor>
  class executor_binder
      ...
      : public detail::executor_binder_result_type<T>,
        ...
        private detail::
            executor_binder_base<T, Executor, uses_executor<T, Executor>::value>
#endif // !defined(GENERATING_DOCUMENTATION)
  {
     ...
     * target object:
     *
     * @li if @c T is a pointer to function type, @c result_type is a synonym
    ...
    /**
     * This constructor is only valid if the type @c T is constructible from
     * type
     * @c U.
     */
    template <typename U>
    executor_binder(executor_arg_t, const executor_type &e, ASIO_MOVE_ARG(U) u)
    ...
    operator()(ASIO_MOVE_ARG(Args)... args) {
    ...
    template <typename... Args>
    typename result_of<T(Args...)>::type
    operator()(ASIO_MOVE_ARG(Args)... args) const {
      return this->target_(ASIO_MOVE_CAST(Args)(args)...);
    }
...
#define ASIO_PRIVATE_BIND_EXECUTOR_CALL_DEF(n)                         \
  template <ASIO_VARIADIC_TPARAMS(n)>                                  \
  result_type_or_void operator()(ASIO_VARIADIC_MOVE_PARAMS(n)) {       \
    return this->target_(ASIO_VARIADIC_MOVE_ARGS(n));                  \
  }                                                                    \
                                                                       \
  template <ASIO_VARIADIC_TPARAMS(n)>                                  \
  result_type_or_void operator()(ASIO_VARIADIC_MOVE_PARAMS(n)) const { \
    return this->target_(ASIO_VARIADIC_MOVE_ARGS(n));                  \
  }                                                                    \
  /**/
    ASIO_VARIADIC_GENERATE(ASIO_PRIVATE_BIND_EXECUTOR_CALL_DEF)
#undef ASIO_PRIVATE_BIND_EXECUTOR_CALL_DEF
  ...
  template <typename T, typename Executor>
  struct uses_executor<executor_binder<T, Executor>, Executor> : true_type {};
  ...
  struct handler_type<executor_binder<T, Executor>, Signature> {
    typedef executor_binder<typename handler_type<T, Signature>::type, Executor>
        type;
  };
