...
namespace asio {
  namespace windows {
    ...
    /// I/O.
    /**
     * A special-purpose smart pointer used to wrap an application handler so
     * that it can be passed as the LPOVERLAPPED argument to overlapped I/O
     * functions.
     *
     * @par Thread Safety
    ...
    class overlapped_ptr : private noncopyable {
      ...
      /// object.
      template <typename Handler>
      void reset(asio::io_context &io_context, ASIO_MOVE_ARG(Handler) handler) {
