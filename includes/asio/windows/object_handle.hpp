...
namespace asio {
  namespace windows {
     ...
     */
    class object_handle : ASIO_SVC_ACCESS basic_io_object<ASIO_SVC_T> {
      public:
       ...
       *
       * @param io_context The io_context object that the object handle will use
      ...
      object_handle(asio::io_context &io_context,
        ...
        this->get_service().assign(
            this->get_implementation(), native_handle, ec);
       ...
       * if constructed using the @c object_handle(io_context&) constructor.
       */
       ...
       * layers. Since an object_handle cannot contain any further layers, it
       ...
       * of layers. Since an object_handle cannot contain any further layers, it
       * simply returns a reference to itself.
       *
      ...
      /// Close the handle.
      /**
       * This function is used to close the handle. Any asynchronous read or
       ...
       * the handle. This is intended to allow access to native handle
       ...
       * signalled state. This function blocks and does not return until the
       * object handle has been set to the signalled state.
       *
       * @throws asio::system_error Thrown on failure.
       */
      void wait() {
       ...
       *
       * @param handler The handler to be called when the object handle is set
       * to the signalled state. Copies will be made of the handler as required.
      ...
      async_wait(ASIO_MOVE_ARG(WaitHandler) handler) {
        asio::async_completion<WaitHandler, void(asio::error_code)> init(
            handler);
