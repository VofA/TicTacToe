...
#define ASIO_PRIVATE_CONNECT_DEF(n)                                           \
  ...
  basic_socket_streambuf *connect(ASIO_VARIADIC_BYVAL_PARAMS(n)) {            \
    ...
    typedef typename Protocol::resolver resolver_type;                        \
    resolver_type resolver(socket().get_executor().context());                \
    connect_to_endpoints(resolver.resolve(ASIO_VARIADIC_BYVAL_ARGS(n), ec_)); \
    return !ec_ ? this : 0;                                                   \
  }                                                                           \
  /**/
...
namespace asio {
  namespace detail {
    ...
    // initialised prior to the basic_socket_streambuf's basic_socket base
    // class.
    class socket_streambuf_io_context {
      protected:
      socket_streambuf_io_context(io_context *ctx) : default_io_context_(ctx) {
      }
    ...
    // buffers are constructed prior to the basic_socket_streambuf's
    // basic_socket base class. This makes moving the socket is the last
    // potentially throwing step in the streambuf's move constructor, giving the
    // constructor a strong exception safety guarantee.
    class socket_streambuf_buffers {
      protected:
      socket_streambuf_buffers()
          : get_buffer_(buffer_size), put_buffer_(buffer_size) {
      }
...
#define ASIO_BASIC_SOCKET_STREAMBUF_FWD_DECL
  ...
  template <typename Protocol ASIO_SVC_TPARAM_DEF1(
    ...
    defined(ASIO_USE_BOOST_DATE_TIME_FOR_SOCKET_IOSTREAM)
            ...
            typename WaitTraits = time_traits<Clock> ASIO_SVC_TPARAM1_DEF2(
                = deadline_timer_service<Clock, WaitTraits>)>
#else  // defined(ASIO_HAS_BOOST_DATE_TIME)
       // && defined(ASIO_USE_BOOST_DATE_TIME_FOR_SOCKET_IOSTREAM)
            typename Clock      = chrono::steady_clock,
            typename WaitTraits = wait_traits<Clock> ASIO_SVC_TPARAM1_DEF1(
                = steady_timer::service_type)>
#endif // defined(ASIO_HAS_BOOST_DATE_TIME)
       // && defined(ASIO_USE_BOOST_DATE_TIME_FOR_SOCKET_IOSTREAM)
...
/// Iostream streambuf for a socket.
  ...
  template <typename Protocol ASIO_SVC_TPARAM,
            typename Clock,
            typename WaitTraits ASIO_SVC_TPARAM1>
...
#endif // defined(ASIO_NO_DEPRECATED) || defined(GENERATING_DOCUMENTATION)
  {
    ...
    /// Establish a connection.
    /**
     * This function automatically establishes a connection based on the
     * supplied resolver query parameters. The arguments are used to construct a
     * resolver query object.
     *
     * @return \c this if a connection was successfully established, a null
     * pointer otherwise.
     */
    ...
    /// to now.
    /**
     * @return A relative time value representing the stream buffer's expiry
     * time.
     */
    ...
    int_type overflow(int_type c) {
      ...
      if (put_buffer_.empty()) {
        ...
        output_buffer =
            asio::buffer(pbase(), (pptr() - pbase()) * sizeof(char_type));
      }
      ...
      while (output_buffer.size() > 0) {
        ...
        if (!socket().native_non_blocking())
          socket().native_non_blocking(true, ec_);
        detail::buffer_sequence_adapter<const_buffer, const_buffer> bufs(
            output_buffer);
    ...
    basic_socket_streambuf &
    operator=(const basic_socket_streambuf &) ASIO_DELETED;
    ...
    int timeout() const {
      int64_t msec =
          traits_helper::to_posix_duration(
              traits_helper::subtract(expiry_time_, traits_helper::now()))
              .total_milliseconds();
    ...
    void connect_to_endpoints(EndpointIterator begin, EndpointIterator end) {
      ...
      for (EndpointIterator i = begin; i != end; ++i) {
        ...
        if (!socket().native_non_blocking())
          socket().native_non_blocking(true, ec_);
        detail::socket_ops::connect(
            socket().native_handle(), ep.data(), ep.size(), ec_);
        ...
        if (detail::socket_ops::getsockopt(socket().native_handle(),
                                           0,
                                           SOL_SOCKET,
                                           SO_ERROR,
                                           &connect_error,
                                           &connect_error_len,
                                           ec_) == detail::socket_error_retval)
          return;

        // Check the result of the connect operation.
        ec_ =
            asio::error_code(connect_error, asio::error::get_system_category());
        if (!ec_)
          return;
      }
#endif // defined(ASIO_WINDOWS_RUNTIME)
